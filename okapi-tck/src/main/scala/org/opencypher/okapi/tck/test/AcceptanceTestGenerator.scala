...
case class AcceptanceTestGenerator(
  ...
  def generateGivenScenarios(
    outDir: File,
    resFiles: Array[File],
    keyWords: Array[String] = Array.empty
  ): Unit = {
    ...
    generateClassFile(
      "specialWhiteCases",
      wantedWhiteScenarios,
      black = false,
      outDir
    )
    generateClassFile(
      "specialBlackCases",
      wantedBlackScenarios,
      black = true,
      outDir
    )
  }
  ...
  private def generateClassFile(
    ...
    val packageName =
      if (black) packageNames.get("black") else packageNames.get("white")

    val testCases = scenarios
      .filterNot(
        _.name.equals("Failing on incorrect unicode literal")
      ) // produced test couldn't be compiled
  ...
  private def getScenarios(resFiles: Array[File]): ScenariosFor = {
    ...
    val resFileNames = resFiles
      .map(_.getPath)
      .filterNot(name => name.contains(".feature") || name.contains(".scala"))
  ...
  private def generateTest(scenario: Scenario, black: Boolean): String = {
    ...
    val initQueries = initSteps.collect { case Execute(query, InitQuery, _) =>
      s"${alignString(query)}"
    }
    ...
    if (black)
      s"""  it("${scenario.name}") {
         ...
         |    }) match{
         |      case Success(_) =>
         |        throw new RuntimeException("${if (expectsError)
          "False-positive as probably wrong error"
        else "A blacklisted scenario works"}")
         |      case Failure(_) =>
         |    }
         |  }
      """.stripMargin
  ...
  private def stepsToString(steps: List[(Step, Int)]): String = {
    case class stepContext(
      ExecQueryStepNr: Option[Int],
      ParameterStepNr: Option[Int],
      graphStateStepNr: Option[Int]
    )

    steps
      .foldLeft((stepContext(None, None, None), "")) {
        case ((context, accString), (Parameters(p, _), stepNr)) =>
          val stepString =
            s"val parameter$stepNr = ${tckCypherMapToOkapiCreateString(p)}"
          stepContext(
            context.ExecQueryStepNr,
            Some(stepNr),
            context.graphStateStepNr
          ) -> (accString + s"\n    $stepString")
        case ((context, accString), (_: Measure, stepNr)) =>
          if (checkSideEffects) {
            val stepString =
              s"val beforeState$stepNr = SideEffectOps.measureState(TCKGraph($graphFactoryName,graph))"
            stepContext(
              context.ExecQueryStepNr,
              context.ParameterStepNr,
              Some(stepNr)
            ) -> (accString + s"\n    $stepString")
           ...
           """
            ...
            case _ =>
              ...
              throw NotImplementedException(
                "Side Effect Queries specified inside the scenario are not supported yet"
              )
          }
        case (
              (context, accString),
              (ExpectResult(expectedResult: CypherValueRecords, _, sorted), _)
            ) =>
          val resultNumber = context.ExecQueryStepNr match {
            case Some(stepNr) => stepNr
            case None =>
              throw new IllegalStateException(
                s"no query found to compare result $expectedResult"
              )
          }
          ...
          val stepString =
            s"""|val result${resultNumber}ValueRecords = convertToTckStrings(result$resultNumber.records).asValueRecords
              |val expected${resultNumber}ValueRecords = CypherValueRecords(List(${expectedResult.header
                .map(escapeString)
                .mkString(",")}),
              |  List(${expectedResult.rows
                .map(tckCypherMapToTCKCreateString)
                .mkString(s", \n           ")}))
              |
        ...
        case (
              (context, accString),
              (ExpectError(errorType, errorPhase, detail, _), _)
            ) =>
          val stepNumber = context.ExecQueryStepNr match {
            case Some(stepNr) => stepNr
            case None =>
              throw new IllegalStateException(
                s"no query found to check for thrown error $detail"
              )
          }
        ...
        case ((context, accString), (SideEffects(expected, _), _)) =>
          ...
          val stepString = if (checkSideEffects) {
            val contextStep = context.graphStateStepNr match {
              ...
              case None =>
                throw new IllegalStateException(
                  s"no graph state found to check side effects"
                )
            }
            s"""|val afterState$contextStep = SideEffectOps.measureState(TCKGraph($graphFactoryName,graph))
              |(beforeState$contextStep diff afterState$contextStep) shouldEqual ${diffToCreateString(
                 expected
               )}
           """.stripMargin
          } else if (relevantSideEffects.nonEmpty)
            s"fail //due to ${relevantSideEffects.mkString(" ")} sideEffects expected"
          else ""
          context -> (accString + s"\n    $stepString")
