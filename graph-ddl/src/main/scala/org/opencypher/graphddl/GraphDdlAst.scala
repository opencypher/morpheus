...
object NodeTypeDefinition {
  def apply(elementTypes: String*): NodeTypeDefinition = NodeTypeDefinition(
    elementTypes.toSet
  )
}
...
object RelationshipTypeDefinition {
  def apply(
    ...
    RelationshipTypeDefinition(
      NodeTypeDefinition(startNodeElementType),
      Set(elementType),
      NodeTypeDefinition(endNodeElementType)
    )

  def apply(startNodeElementTypes: String*)(
    elementTypes: String*
  )(endNodeElementTypes: String*): RelationshipTypeDefinition =
    RelationshipTypeDefinition(
      NodeTypeDefinition(startNodeElementTypes.toSet),
      elementTypes.toSet,
      NodeTypeDefinition(endNodeElementTypes.toSet)
    )
}
...
case class RelationshipTypeDefinition(
  ...
  override def toString: String =
    s"$startNodeType-[${elementTypes.mkString(",")}]->$endNodeType"
}
...
case class NodeToViewDefinition(
  viewId: List[String],
  override val maybePropertyMapping: Option[
    PropertyToColumnMappingDefinition
  ] = None
) extends GraphDdlAst
    with ElementToViewDefinition
