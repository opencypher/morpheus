...
trait Cypher10Acceptance[Session <: CypherSession, Graph <: PropertyGraph] {
  ...
  def testedScenarios: TableFor1[Cypher10Feature] = cypher10Scenarios
  ...
  def register(namespace: Namespace)(implicit ctx: TestContext): Unit = {
    session.registerSource(namespace, ctx.pgds)
  }

  def storeAndRegister(gn: GraphName, namespace: Namespace)(implicit ctx: TestContext): Unit = {
    pgds.store(gn, graph(gn))
    register(namespace)
  }

  lazy val cypher10Scenarios = Table(
    "Read-only scenario",

    Cypher10Feature("Session.registerSource") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
      session.catalog.source(ns).hasGraph(g1) shouldBe true
      session.catalog.source(ns).hasGraph(GraphName("foo")) shouldBe false
    },

    Cypher10Feature("PropertyGraphDataSource.hasGraph") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
      session.catalog.source(ns).hasGraph(g1) shouldBe true
      session.catalog.source(ns).hasGraph(GraphName("foo")) shouldBe false
    },

    Cypher10Feature("PropertyGraphDataSource.graphNames") { implicit ctx: TestContext =>
      pgds.graphNames should be(Set.empty)
      storeAndRegister(g1, ns)
      session.catalog.source(ns).graphNames should be(Set(g1))
    },

    Cypher10Feature("PropertyGraphDataSource.graph") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
      session.catalog.source(ns).graph(g1)
    },

    Cypher10Feature("Correct schema for graph #1") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
    ...
    Cypher10Feature("PropertyGraphDataSource: correct node/rel count for graph #1") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
    ...
    Cypher10Feature("Cypher query directly on graph #1") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
    ...
    Cypher10Feature("Cypher query on session") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
      session.cypher(s"FROM GRAPH $ns.$g1 MATCH (b:B) RETURN b.type, b.size").records.iterator.toBag should equal(Bag(
    ...
    Cypher10Feature("Scans over multiple labels") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
      session.cypher(s"FROM GRAPH $ns.$g1 MATCH (n) RETURN n.name, n.size").records.iterator.toBag should equal(Bag(
    ...
    Cypher10Feature("multi-hop paths") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
      session.cypher(s"FROM GRAPH $ns.$g1 MATCH (a)-[r1]->(b)-[r2]->(c) RETURN r1.since, r2.since, type(r2)").records.iterator.toBag should equal(Bag(
        CypherMap("r1.since" -> 2004, "r2.since" -> 2005, "type(r2)" -> "R"),
        CypherMap("r1.since" -> 2005, "r2.since" -> 2006, "type(r2)" -> "S")
      ))
    },

    Cypher10Feature("store a graph") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
      val gn = GraphName("storedGraph")
      Try(session.cypher(s"CATALOG CREATE GRAPH $ns.storedGraph2 { FROM GRAPH $ns.$g1 RETURN GRAPH }")) match {
        case Success(_) =>
    ...
    Cypher10Feature("store a constructed graph") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
      val gn = GraphName("storedGraph")
      Try(session.cypher(
        s"""
    ...
    Cypher10Feature("store nodes without labels") { implicit ctx: TestContext =>
      register(ns)
      val gn = GraphName("storedGraph")
      Try(session.cypher(
        s"""
    ...
    Cypher10Feature("store European Latin unicode labels, rel types, property keys, and property values") { implicit
      ctx: TestContext =>
      register(ns)
      val gn = GraphName("storedGraph")
      Try(session.cypher(
    ...
    Cypher10Feature("property with property key `id`") { implicit ctx: TestContext =>
      register(ns)
      val gn = GraphName("storedGraph")
      Try(session.cypher(
        s"""
    ...
    Cypher10Feature("store a union graph") { implicit ctx: TestContext =>
      register(ns)
    ...
    Cypher10Feature("repeat CONSTRUCT ON") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
    ...
    Cypher10Feature("drop a graph") { implicit ctx: TestContext =>
      storeAndRegister(g1, ns)
