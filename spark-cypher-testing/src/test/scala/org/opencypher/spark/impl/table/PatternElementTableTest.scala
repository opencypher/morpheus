...
class PatternElementTableTest extends CAPSTestSuite {

  private val nodeMapping: ElementMapping = NodeMappingBuilder
    .withSourceIdKey("ID")
    .withImpliedLabel("A")
    .withImpliedLabel("B")
  ...
  private val relMapping: ElementMapping = RelationshipMappingBuilder
    .withSourceIdKey("ID")
  ...
  ignore("throws an IllegalArgumentException when a relationship table does not have the expected column ordering") {
    ...
    an[IllegalArgumentException] should be thrownBy {
      CAPSElementTable.create(relMapping, df)
    }
  }
  ...
  it("NodeTable should not accept wrong source id key type (should be compatible to LongType)") {
    val e = the[IllegalArgumentException] thrownBy {
      ...
      CAPSElementTable.create(nodeMapping, df)
    }
