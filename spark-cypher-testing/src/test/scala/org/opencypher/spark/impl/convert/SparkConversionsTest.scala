...
class SparkConversionsTest extends BaseTestSuite {
  ...
  it("should produce the correct StructField for non-nested types") {
    ...
    CTBigDecimal(12).toStructField("foo") should equal(StructField("foo", DataTypes.createDecimalType(38,12), nullable = false))
    CTBigDecimalOrNull(10).toStructField("foo") should equal(StructField("foo", DataTypes.createDecimalType(10,10), nullable = true))
