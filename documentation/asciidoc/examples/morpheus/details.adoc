...
* <<example-detail-morpheus-session, Create a Morpheus session>>.
...
[discrete]
[[example-detail-morpheus-session]]
== Morpheus session

The first procedure in most Morpheus applications is the creation of a Morpheus session.
...
Node and relationship mappings are used to explicitly define which DataFrame column stores which specific element component.
The element components are: identifiers, properties, node labels (optional), and relationship types.
