...
class LogicalOperatorProducer {
  ...
  def plan(ir: QueryRepresentation[Expr]): LogicalOperator = {
    val root = ir.root
  ...
  private def planProjections(in: LogicalOperator, exprs: Set[Expr])(implicit tokens: TokenDefs) = {
    exprs.foldLeft(in) {
      case (acc, p: Property) =>
        Project(p, acc)
  ...
  private def wherePlanner(in: LogicalOperator, where: Where[Expr])(implicit tokens: TokenDefs) = {
  ...
  private def nodePlan(given: Given)(implicit tokens: TokenDefs): (LogicalOperator, Given) = {
