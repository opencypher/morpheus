...
import CypherTypes.CTString
import org.opencypher.spark.impl.StdSlot
import org.opencypher.spark.{CypherTypes, BinaryRepresentation, CypherValue, EmbeddedRepresentation}
...
case object ProductEncoderFactory {
  ...
  def createEncoder(slots: Seq[StdSlot])(implicit session: SparkSession): ExpressionEncoder[Product] = {
    val encoders = slots.map {
      ...
      case StdSlot(_, CTString, _, EmbeddedRepresentation(StringType)) => session.implicits.newStringEncoder
