...
object RefCollection {
  ...
  abstract class AbstractRegister[R, K, D] extends Register[RefCollection[D]] {
    ...
    override def get(collection: RefCollection[D], ref: Ref): Option[Def] = {
      val idx = id(ref)
    ...
    override def find(collection: RefCollection[D], defn: Def): Option[Ref] =
      lookup(collection, key(defn)).filter { ref => collection.elts(id(ref)) == defn }

    override def lookup(collection: RefCollection[D], k: Key): Option[Ref] = {
