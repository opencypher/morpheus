...
/**
  * Demonstrates usage patterns where Cypher and SQL can be interleaved in the same processing
  * chain, by using the tabular output of a Cypher query as a SQL table, and using the output of a
  * SQL query as an input driving table for a Cypher query.
  */
object CypherSQLRoundtripExample extends App {
  ...
  morpheus.registerSource(
    ...
    GraphSources.fs(rootPath = graphDir).csv
  )
  ...
  val result2 = morpheus.catalog
    .graph("myDataSource.products")
    .cypher(
      s"""
       |MATCH (c:Customer {name: name})-->(p:Product)
       |RETURN c.name, age, p.title
     """.stripMargin,
      drivingTable = Some(sqlResults)
    )
