...
/**
  ...
  *
  * 1) Loads the raw CSV files into Hive tables 2) Normalizes tables according to the LDBC schema
  * (i.e. place -> [City, Country, Continent] 3) Generates a Graph DDL script based on LDBC naming
  * conventions (if not already existing) 4) Initializes a SQL PGDS based on the generated Graph DDL
  * file 5) Runs a Cypher query over the LDBC graph in Spark
  *
  * More detail about the LDBC SNB data generator are available under
