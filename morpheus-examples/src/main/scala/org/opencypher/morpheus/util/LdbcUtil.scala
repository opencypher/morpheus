...
object LdbcUtil {
  ...
  case class EdgeType(
    ...
    def edgeTable: String =
      s"${startNodeType.nodeTable}_${edgeType}_${endNodeType.nodeTable}"
    override def toString: String = s"$startNodeType-[$edgeType]->$endNodeType"
  }

  /**
    * Generates a Graph Ddl script for the LDBC graph. The extraction of nodes and relationship
    * types and their corresponding table/view mappings is based on the naming conventions used by
    * the LDBC data generator.
    *
    * @param datasource
    *   the data source used in the SET SCHEMA definition
    * @param database
    *   the Hive database to get the table information from
    * @param spark
    *   a Spark instance
    * @return
    *   a Graph Ddl string
    */
  def toGraphDDL(datasource: String, database: String)(implicit
    spark: SparkSession
  ): String = {
    // select all tables (including views)
    val tableNames = spark
    ...
    val elementTypes =
      toElementType(database, nodeTables ++ edgeTables).toSeq.sorted

    val nodeMappings =
      nodeTables.map(nodeTable => s"(${nodeTable.toNodeLabel}) FROM $nodeTable")
    val edgeMappings = edgeTables
      .map(_.split("_"))
      .map(tokens => EdgeType(NodeType(tokens(0)), tokens(1), NodeType(tokens(2))))
      .map {
        case et @ EdgeType(
              snt @ NodeType(startNodeTable),
              _,
              ent @ NodeType(endNodeTable)
            ) =>
          ...
          val startNodes =
            s"$snt FROM $startNodeTable node JOIN ON $startJoinExpr"
    ...
    s"""-- generated by ${getClass.getSimpleName
        .dropRight(1)} on ${Calendar.getInstance().getTime}
       |
  ...
  private def toElementType(database: String, tableNames: Set[String])(implicit
    spark: SparkSession
  ): Set[String] =
    tableNames.map { tableName =>
      val tableInfos =
        spark.sql(s"DESCRIBE TABLE $database.$tableName").collect()
