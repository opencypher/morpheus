...
object TemporalTypesHelper {
  ...
  val dateByQuarterIdentifiers: Seq[String] =
    Seq("year", "quarter", "dayofquarter")
  ...
  val timeFormatters: Seq[DateTimeFormatter] = Seq(
    ...
    new DateTimeFormatterBuilder()
      .appendPattern("HHmmss.SSSSSSSSS")
      .toFormatter,
  ...
  private def parseDateMap(map: Map[String, Int]): LocalDate = {
    ...
    if (!sanitizedMap.contains("year"))
      throw IllegalArgumentException(
        "the key `year` needs to be set",
        map.keys.mkString(", ")
      )
    ...
    if (sanitizedMap.keySet.contains("week")) {
      ...
      LocalDate.MIN
        ...
        .`with`(
          ...
          sanitizedMap.getOrElse("dayofweek", 1).toLong
        )
      ...
      LocalDate.MIN
        ...
        .`with`(
          ...
          sanitizedMap.getOrElse("dayofquarter", 1).toLong
        )
  ...
  private def parseDateString(str: String): LocalDate = {
    ...
    matchingDateFormats.find(_.isDefined).flatten match {
      ...
      case None =>
        throw IllegalArgumentException("a valid date construction string", str)
    }
  }
  ...
  private def parseTimeString(str: String): LocalTime = {
    ...
    matchingTimeFormats.find(_.isDefined).flatten match {
      ...
      case None =>
        throw IllegalArgumentException("a valid time construction string", str)
    }
  }

  private def checkSignificanceOrder(
    inputMap: Map[String, _],
    keys: Seq[String]
  ): Unit = {
    val validOrder = keys
  ...
  def sanitizeMap(map: Map[String, Int]): Map[String, Int] = map.map { case (key, value) =>
