...
/**
  ...
  *   number of seconds
  * @param nanos
  *   normalized number of nanoseconds spanning fractions of a second
  */
class Duration protected (
  ...
  def toJava: (java.time.Period, java.time.Duration) = {
    ...
    val duration =
      java.time.Duration.ofSeconds(seconds).plus(nanos, ChronoUnit.NANOS)
  ...
  private lazy val COMPARATOR: Comparator[Duration] = {
    ...
    implicit def toLongFunction[T](f: T => Long): ToLongFunction[T] =
...
object Duration {
  ...
  def apply(
    ...
    val normalizedSeconds =
      hours * SECONDS_PER_HOUR + minutes * SECONDS_PER_MINUTE + seconds + nanoSum / NANOS_PER_SECOND
  ...
  def parse(durationString: String): Duration = {
    ...
    durationRegex.findFirstMatchIn(durationString) match {
      case Some(m) =>
        val superSecondMap =
          Seq("years", "months", "weeks", "days", "hours", "minutes")
      ...
      case _ =>
        throw IllegalArgumentException(
          "a valid duration construction string",
          durationString
        )
    }
  }
