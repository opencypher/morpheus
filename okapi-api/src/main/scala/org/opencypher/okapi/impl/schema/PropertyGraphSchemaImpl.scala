...
object PropertyGraphSchemaImpl {
  ...
  implicit def rw: ReadWriter[PropertyGraphSchema] =
    readwriter[Value].bimap[PropertyGraphSchema](
      schema => {
        ...
        val versionString: String = json.obj(VERSION) match {
          ...
          case other =>
            throw SchemaException(
              s"Expected Version to be a String or a Number but got $other"
            )
        }
        val version = Version(versionString)
        if (!version.compatibleWith(PropertyGraphSchema.CURRENT_VERSION))
          throw SchemaException(s"Incompatible Schema version: $version")

        val labelPropertyMap =
          read[LabelPropertyMap](json.obj(LABEL_PROPERTY_MAP))
        val relTypePropertyMap =
          read[RelTypePropertyMap](json.obj(REL_TYPE_PROPERTY_MAP))
        val explicitSchemaPatterns = json match {
          case Obj(m) if m.keySet.contains(SCHEMA_PATTERNS) =>
            read[Set[SchemaPattern]](json.obj(SCHEMA_PATTERNS))
          case _ => Set.empty[SchemaPattern]
        }
        val nodeKeys = json match {
          case Obj(m) if m.keySet.contains(NODE_KEYS) =>
            read[Map[String, Set[String]]](json.obj(NODE_KEYS))
          case _ => Map.empty[String, Set[String]]
        }
        val relKeys = json match {
          case Obj(m) if m.keySet.contains(REL_KEYS) =>
            read[Map[String, Set[String]]](json.obj(REL_KEYS))
          case _ => Map.empty[String, Set[String]]
        }
  ...
  implicit def lpmRw: ReadWriter[LabelPropertyMap] =
    readwriter[Value].bimap[LabelPropertyMap](
      labelPropertyMap =>
        labelPropertyMap.map { case (labelCombo, propKeys) =>
          Obj(LABELS -> writeJs(labelCombo), PROPERTIES -> writeJs(propKeys))
        },
      json =>
        json.arr.map { value =>
          read[Set[String]](value.obj(LABELS)) -> read[PropertyKeys](
            value.obj(PROPERTIES)
          )
        }.toMap
    )

  implicit def rpmRw: ReadWriter[RelTypePropertyMap] =
    readwriter[Value].bimap[RelTypePropertyMap](
      relTypePropertyMap =>
        relTypePropertyMap.map { case (relType, propKeys) =>
          Obj(REL_TYPE -> writeJs(relType), PROPERTIES -> writeJs(propKeys))
        },
      json =>
        json.arr.map { value =>
          read[String](value.obj(REL_TYPE)) -> read[PropertyKeys](
            value.obj(PROPERTIES)
          )
        }.toMap
    )
...
final case class PropertyGraphSchemaImpl(
  ...
  override lazy val impliedLabels: ImpliedLabels = {
    val implications =
      self.labelCombinations.combos.foldLeft(Map.empty[String, Set[String]]) {
        case (currentMap, combo) =>
          combo.foldLeft(currentMap) { case (innerMap, label) =>
            innerMap.get(label) match {
              case Some(innerCombo) =>
                innerMap.updated(label, (innerCombo intersect combo) - label)
  ...
  override def nodePropertyKeys(labelCombination: Set[String]): PropertyKeys =
  ...
  override def nodePropertyKeyType(
  ...
  override def nodePropertyKeysForCombinations(
  ...
  override def relationshipPropertyKeyType(
    types: Set[String],
    key: String
  ): Option[CypherType] = {
  ...
  override def relationshipPropertyKeysForTypes(
    knownTypes: Set[String]
  ): PropertyKeys = {
    val relevantTypes =
      if (knownTypes.isEmpty) relationshipTypes else knownTypes
  ...
  override def relationshipPropertyKeys(typ: String): PropertyKeys =
    relTypePropertyMap.properties(typ)
  ...
  override def schemaPatternsFor(
    ...
    val possibleSourcePatterns =
      schemaPatterns.filter(p => knownSourceLabels.subsetOf(p.sourceLabelCombination))
    val possibleRelTypePatterns =
      possibleSourcePatterns.filter(p => knownRelTypes.contains(p.relType) || knownRelTypes.isEmpty)
    val possibleTargetPatterns =
      possibleRelTypePatterns.filter(p => knownTargetLabels.subsetOf(p.targetLabelCombination))

    possibleTargetPatterns
  }

  override def withNodePropertyKeys(
    ...
    val propertyKeys =
      if (labelPropertyMap.labelCombinations(labelCombination)) {
        computePropertyTypes(
          labelPropertyMap.properties(labelCombination),
          keys
        )
      } else {
  ...
  override def withNodeKey(
    ...
    val propertyKeys = nodePropertyKeysForCombinations(
      combinationsFor(Set(label))
    )

    if (!nodeKey.subsetOf(propertyKeys.keySet)) {
      throw SchemaException(s"""|Invalid node key for schema
            |$pretty
            |Not all combinations that contain `$label` have all the properties for ${nodeKey
                                 .mkString("[", ", ", "]")}.
            |Available keys: ${propertyKeys.keySet.mkString(
                                 "[",
                                 ", ",
                                 "]"
                               )}""".stripMargin)
    ...
    val nullableProperties = propertyKeys.filter { case (key, tpe) =>
      nodeKey.contains(key) && tpe.isNullable
    }

    if (nullableProperties.nonEmpty) {
      throw SchemaException(s"""|Invalid node key for schema
            |$pretty
            |Properties ${nullableProperties.keySet.mkString(
                                 "[",
                                 ", ",
                                 "]"
                               )} have nullable types.
  ...
  override def withRelationshipKey(
    relationshipType: String,
    relationshipKey: Set[String]
  ): PropertyGraphSchema = {
    if (!relationshipTypes.contains(relationshipType)) {
      throw SchemaException(s"""|Invalid relationship key for schema
            |$pretty
            |Unknown relationship type `$relationshipType`.
            |Should be one of: ${relationshipTypes.mkString(
                                 "[",
                                 ", ",
                                 "]"
                               )}.""".stripMargin)
    ...
    if (!relationshipKey.subsetOf(propertyKeys.keySet)) {
      throw SchemaException(s"""|Invalid relationship key for schema
            |$pretty
            |Relationship type `$relationshipType` does not have all the properties for ${relationshipKey
                                 .mkString("[", ", ", "]")}.
            |Available keys: ${propertyKeys.keySet.mkString(
                                 "[",
                                 ", ",
    ...
    val nullableProperties = propertyKeys.filter { case (key, tpe) =>
      relationshipKey.contains(key) && tpe.isNullable
    }
    ...
    if (nullableProperties.nonEmpty) {
      throw SchemaException(
        s"""|Invalid relationship key for schema
            |$pretty
            |Properties ${nullableProperties.keySet
             .mkString("[", ", ", "]")} have nullable types.
  ...
  private def computePropertyTypes(
    existing: PropertyKeys,
    input: PropertyKeys
  ): PropertyKeys = {
    // Map over input keys to calculate join of type with existing type
    val keysWithJoinedTypes = input.map { case (key, propType) =>
    ...
    val propertiesMarkedOptional = existing
      .filterKeys(k => !input.contains(k))
      .foldLeft(keysWithJoinedTypes) { case (map, (key, propTyp)) =>
  ...
  override def withRelationshipPropertyKeys(
    typ: String,
    keys: PropertyKeys
  ): PropertyGraphSchema = {
    if (relationshipTypes contains typ) {
      val updatedTypes =
        computePropertyTypes(relTypePropertyMap.properties(typ), keys)
  ...
  override def withSchemaPatterns(
    patterns: SchemaPattern*
  ): PropertyGraphSchema = {
    patterns.foreach { p =>
      if (!labelCombinations.combos.contains(p.sourceLabelCombination))
        throw SchemaException(
          s"Unknown source node label combination: `${p.sourceLabelCombination}`. Should be one of: ${labelCombinations.combos
              .mkString("[", ",", "]")}"
        )
      if (!relationshipTypes.contains(p.relType))
        throw SchemaException(
          s"Unknown relationship type: `${p.relType}`. Should be one of ${relationshipTypes
              .mkString("[", ",", "]")}"
        )
      if (!labelCombinations.combos.contains(p.targetLabelCombination))
        throw SchemaException(
          s"Unknown target node label combination: `${p.targetLabelCombination}`. Should be one of: ${labelCombinations.combos
              .mkString("[", ",", "]")}"
        )
    }
  ...
  override def ++(other: PropertyGraphSchema): PropertyGraphSchemaImpl = {
    val conflictingLabels =
      labelPropertyMap.labelCombinations intersect other.labelPropertyMap.labelCombinations
    val nulledOut =
      conflictingLabels.foldLeft(Map.empty[Set[String], PropertyKeys]) { case (acc, next) =>
        val keys = computePropertyTypes(
          labelPropertyMap.properties(next),
          other.labelPropertyMap.properties(next)
        )
        acc + (next -> keys)
      }
    val newLabelPropertyMap =
      labelPropertyMap |+| other.labelPropertyMap |+| nulledOut

    val conflictingRelTypes =
      relationshipTypes intersect other.relationshipTypes
    val nulledRelProps =
      conflictingRelTypes.foldLeft(Map.empty[String, PropertyKeys]) { case (acc, next) =>
        val keys = computePropertyTypes(
          relTypePropertyMap.properties(next),
          other.relTypePropertyMap.properties(next)
        )
        acc + (next -> keys)
      }
    val newRelTypePropertyMap =
      relTypePropertyMap |+| other.relTypePropertyMap |+| nulledRelProps

    val newExplicitSchemaPatterns =
      explicitSchemaPatterns ++ other.explicitSchemaPatterns
  ...
  def forNode(labelConstraints: Set[String]): PropertyGraphSchema = {
    val requiredLabels = {
      ...
      val impliedLabels =
        this.impliedLabels.transitiveImplicationsFor(explicitLabels)
    ...
    val newLabelPropertyMap: LabelPropertyMap =
      this.labelPropertyMap.filterKeys(possibleLabels.contains)
  ...
  def forRelationship(relType: CTRelationship): PropertyGraphSchema = {
    ...
    val updatedRelTypePropertyMap =
      this.relTypePropertyMap.filterForRelTypes(givenRelTypes)
    val updatedMap = givenRelTypes.foldLeft(updatedRelTypePropertyMap) { case (map, givenRelType) =>
      if (!map.contains(givenRelType))
        map.updated(givenRelType, PropertyKeys.empty)
      else map
    }
    ...
    if (isEmpty) "empty schema"
      ...
      if (labelPropertyMap.labelCombinations.nonEmpty) {
        ...
        labelPropertyMap.labelCombinations.foreach { combo =>
          val labelStr =
            if (combo eq Set.empty) "(no label)"
            else combo.mkString(":", ":", "")
          builder.append(s"\t$labelStr$EOL")
          nodePropertyKeys(combo).foreach { case (key, typ) =>
            builder.append(s"\t\t$key: $typ$EOL")
          }
        }
      ...
      if (impliedLabels.m.exists(_._2.nonEmpty)) {
        ...
        impliedLabels.m.foreach {
          case (label, implications) if implications.nonEmpty =>
            builder.append(
              s":$label -> ${implications.mkString(":", ":", "")}$EOL"
            )
          case _ =>
        }
      } else {
      ...
      if (relationshipTypes.nonEmpty) {
        ...
        relationshipTypes.foreach { relType =>
          ...
          relationshipPropertyKeys(relType).foreach { case (key, typ) =>
            builder.append(s"\t\t$key: $typ$EOL")
          }
        }
        builder.append(s"}$EOL")
      ...
      if (explicitSchemaPatterns.nonEmpty) {
        ...
        explicitSchemaPatterns.foreach(p => builder.append(s"\t$p$EOL"))
        builder.append(s"}$EOL")
      }
  ...
  override def toJson: String =
    upickle.default.write[PropertyGraphSchema](this, indent = 4)
}
