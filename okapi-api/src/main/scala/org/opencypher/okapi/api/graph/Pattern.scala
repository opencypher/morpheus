...
sealed trait Pattern {
  ...
  /**
    ...
    *
    * If exactMatch is true, then two elements can be mapped if they CypherTypes are equal, e.g.
    * both are CTNode with the same label set. Otherwise a mapping exists if the search element type
    * is supertype of the target element type.
    *
    * @param searchPattern
    *   the pattern for which to find the mapping
    * @param exactMatch
    *   controls how elements are matched
    * @return
    *   a bijective mapping between the search pattern and the target pattern
    */
  def findMapping(
    searchPattern: Pattern,
    exactMatch: Boolean
  ): Option[Map[PatternElement, PatternElement]] = {
    if (
      (exactMatch && searchPattern == this) || (!exactMatch && subTypeOf(
        searchPattern
      ))
    ) {
      searchPattern -> this match {
        ...
        case (search: NodeRelPattern, other: NodeRelPattern) =>
          Some(
            Map(
              search.nodeElement -> other.nodeElement,
              search.relElement -> other.relElement
            )
          )
        case (search: TripletPattern, other: TripletPattern) =>
          Some(
            Map(
              search.sourceElement -> other.sourceElement,
              search.relElement -> other.relElement,
              search.targetElement -> other.targetElement
            )
          )
        case _ => None
      }
  ...
  /**
    * Returns true if the current pattern is a sub type of the other pattern. A pattern is subtype
    * of another if there is an bijective mapping between this patterns elements into the other
    * pattern's elements and for every mapping the source element type is a subtype of the target
    * element type.
    *
    * @param other
    *   reference pattern
    * @return
    *   true if this pattern is subtype of the reference pattern
    */
  def subTypeOf(other: Pattern): Boolean

  /**
    * Returns true if the current pattern is a super type of the other pattern. A pattern is super
    * type of another pattern iff the other pattern is subtype of the first pattern.
    *
    * @see
    *   [[org.opencypher.okapi.api.graph.Pattern#subTypeOf]]
    * @param other
    *   reference
    * @return
    *   true if this pattern s supertype of the reference pattern
    */
...
case class NodePattern(nodeType: CTNode) extends Pattern {
  ...
  override def subTypeOf(other: Pattern): Boolean = other match {
    case NodePattern(otherNodeType) =>
      nodeType.withoutGraph.subTypeOf(otherNodeType.withoutGraph)
    case _ => false
  }
}
...
case class RelationshipPattern(relType: CTRelationship) extends Pattern {
  ...
  override def subTypeOf(other: Pattern): Boolean = other match {
    case RelationshipPattern(otherRelType) =>
      relType.withoutGraph.subTypeOf(otherRelType.withoutGraph)
    case _ => false
  }
}
...
case class NodeRelPattern(nodeType: CTNode, relType: CTRelationship) extends Pattern {
  ...
  override def subTypeOf(other: Pattern): Boolean = other match {
    case NodeRelPattern(otherNodeType, otherRelType) =>
      nodeType.withoutGraph.subTypeOf(
        otherNodeType.withoutGraph
      ) && relType.withoutGraph.subTypeOf(otherRelType.withoutGraph)
    case _ => false
  }
}

case class TripletPattern(
  sourceNodeType: CTNode,
  relType: CTRelationship,
  targetNodeType: CTNode
) extends Pattern {
  val sourceElement =
    PatternElement("source_" + DEFAULT_NODE_NAME, sourceNodeType)
  val targetElement =
    PatternElement("target_" + DEFAULT_NODE_NAME, targetNodeType)
  val relElement = PatternElement(DEFAULT_REL_NAME, relType)
