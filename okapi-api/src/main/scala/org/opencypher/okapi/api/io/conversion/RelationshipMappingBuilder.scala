...
object RelationshipMappingBuilder {
  ...
  /**
    ...
    *   from the source data is expected to be a [[scala.Long]] value that is unique among
    *   relationships.
    * @return
  ...
  /**
    * Alias for
    * [[org.opencypher.okapi.api.io.conversion.RelationshipMappingBuilder#withSourceIdKey]].
    *
    * @param sourceIdKey
    *   represents a key to the relationship identifier within the source data. The retrieved value
    *   from the source data is expected to be a [[scala.Long]] value that is unique among
  ...
  /**
    * Creates a RelationshipMapping where property keys match with their corresponding keys in the
    * source data.
    *
    * See [[org.opencypher.okapi.api.io.conversion.RelationshipMappingBuilder]] for further
    * information.
    *
    * @param sourceIdKey
    *   key to access the node identifier in the source data
    * @param sourceStartNodeKey
    *   key to access the start node identifier in the source data
    * @param sourceEndNodeKey
  ...
  def create(
    ...
    properties
      .foldLeft(intermediateMapping) { (mapping, property) =>
  ...
  sealed class MissingSourceStartNodeKey(sourceIdKey: String) {
    ...
    /**
      * @param sourceStartNodeKey
      *   represents a key to the start node identifier within the source data. The retrieved value
      *   from the source data is expected to be a [[scala.Long]] value.
      * @return
      *   incomplete relationship mapping builder
      */
    def withSourceStartNodeKey(
      sourceStartNodeKey: String
    ): MissingSourceEndNodeKey =
      new MissingSourceEndNodeKey(sourceIdKey, sourceStartNodeKey)

    /**
      * Alias for
      * [[org.opencypher.okapi.api.io.conversion.RelationshipMappingBuilder.MissingSourceStartNodeKey#withSourceStartNodeKey]].
      *
      * @param sourceStartNodeKey
      *   represents a key to the start node identifier within the source data. The retrieved value
      *   from the source data is expected to be a [[scala.Long]] value.
  ...
  sealed class MissingSourceEndNodeKey(
    sourceIdKey: String,
    sourceStartNodeKey: String
  ) {
    ...
    /**
      * @param sourceEndNodeKey
      *   represents a key to the end node identifier within the source data. The retrieved value
      *   from the source data is expected to be a [[scala.Long]] value.
      * @return
      *   incomplete relationship mapping builder
      */
    def withSourceEndNodeKey(sourceEndNodeKey: String): MissingRelTypeMapping =
      new MissingRelTypeMapping(
        sourceIdKey,
        sourceStartNodeKey,
        sourceEndNodeKey
      )
  ...
  sealed class MissingRelTypeMapping(
    sourceIdKey: String,
    sourceStartNodeKey: String,
    sourceEndNodeKey: String
  ) {
    ...
    /**
      * @param relType
      *   represents the relationship type for all relationships in the source data
      * @return
      *   relationship mapping builder
      */
...
/**
  ...
  *
  * Represents a mapping from a source with key-based access to relationship components (e.g. a
  * table definition) to a Cypher relationship. The purpose of this class is to define a mapping
  * from an external data source to a property graph.
  *
  * Construct a [[RelationshipMappingBuilder]] starting with [[RelationshipMappingBuilder#on]].
  *
  * @param relationshipIdKey
  *   key to access the node identifier in the source data
  * @param relationshipStartNodeKey
  *   key to access the start node identifier in the source data
  * @param relationshipEndNodeKey
  *   key to access the end node identifier in the source data
  * @param relType
  *   a relationship type
...
final case class RelationshipMappingBuilder(
  ...
  override def build: ElementMapping = {
    ...
    val pattern: RelationshipPattern = RelationshipPattern(
      CTRelationship(relType)
    )

    val properties: Map[PatternElement, Map[String, String]] = Map(
      pattern.relElement -> propertyMapping
    )
  ...
  override protected def validate(): Unit = {
    val idKeys =
      Set(relationshipIdKey, relationshipStartNodeKey, relationshipEndNodeKey)
