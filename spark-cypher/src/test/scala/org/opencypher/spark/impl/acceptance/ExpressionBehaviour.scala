...
class ExpressionBehaviour extends CAPSTestSuite with DefaultGraphInit {
  ...
  describe("properties") {
    ...
    it("handles unknown properties") {
      ...
      // Then
      result.getRecords.toMapsWithCollectedEntities should equal(Bag(CypherMap("a.age" -> null, "a.firstName" -> "Alice")))
    }
  ...
  describe("ANDs") {
    it("can project ands") {
      ...
      graph.cypher(query).getRecords.toMapsWithCollectedEntities should equal(Bag(
        CypherMap("n.v1" -> true)
      ))
    }
  }
