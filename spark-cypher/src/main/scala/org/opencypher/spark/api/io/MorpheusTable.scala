...
import org.opencypher.spark.impl.{MorpheusRecords, RecordBehaviour}
...
case class MorpheusElementTable private[spark](
  ...
  private[spark] def records(implicit morpheus: MorpheusSession): MorpheusRecords = morpheus.records.fromElementTable(elementTable = this)
