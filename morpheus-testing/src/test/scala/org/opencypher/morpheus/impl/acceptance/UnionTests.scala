...
class UnionTests extends MorpheusTestSuite with ScanGraphInit {
  ...
  describe("tabular union all") {
    ...
    it("supports union all with MATCH on nodes") {
      ...
      result.toMaps should equal(
        Bag(
          CypherMap(
            "node" -> MorpheusNode(0, Set("A"), CypherMap("val" -> "foo"))
          ),
          CypherMap(
            "node" -> MorpheusNode(1, Set("B"), CypherMap("bar" -> "baz"))
          )
        )
      )
    }
  ...
  describe("tabular union") {
    ...
    it("supports union with MATCH on nodes") {
      ...
      result.toMaps should equal(
        Bag(
          CypherMap(
            "node1" -> MorpheusNode(0, Set("A"), CypherMap("val" -> "foo")),
            "node2" -> MorpheusNode(1, Set("B"), CypherMap("bar" -> "baz"))
          ),
          CypherMap(
            "node1" -> MorpheusNode(1, Set("B"), CypherMap("bar" -> "baz")),
            "node2" -> MorpheusNode(0, Set("A"), CypherMap("val" -> "foo"))
          )
        )
      )
    }
  ...
  describe("Graph union all") {
    ...
    it("union all fails on graphs with common properties") {
      ...
      e.getMessage should (include("one") and include("test") and include(
        "STRING"
      ) and include("INTEGER"))
    }
  }
