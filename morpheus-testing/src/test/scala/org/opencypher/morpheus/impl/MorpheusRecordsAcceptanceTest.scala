...
class MorpheusRecordsAcceptanceTest
    extends MorpheusTestSuite
    with ScanGraphInit
    with OpenCypherDataFixture {

  private lazy val graph: RelationalCypherGraph[DataFrameTable] = initGraph(
    dataFixture
  )
  ...
  it("convert nodes to CypherMaps") {
    // When
    val result =
      graph.cypher("MATCH (a:Person) WHERE a.birthyear < 1930 RETURN a, a.name")
      ...
      Bag(
        CypherMap(
          "a" -> MorpheusNode(
            0,
            Set("Actor", "Person"),
            CypherMap("birthyear" -> 1910, "name" -> "Rachel Kempson")
          ),
          "a.name" -> "Rachel Kempson"
        ),
        CypherMap(
          "a" -> MorpheusNode(
            1,
            Set("Actor", "Person"),
            CypherMap("birthyear" -> 1908, "name" -> "Michael Redgrave")
          ),
          "a.name" -> "Michael Redgrave"
        ),
        CypherMap(
          "a" -> MorpheusNode(
            10,
            Set("Actor", "Person"),
            CypherMap("birthyear" -> 1873, "name" -> "Roy Redgrave")
          ),
          "a.name" -> "Roy Redgrave"
        )
      )
  ...
  it("convert rels to CypherMaps") {
    ...
    val result = graph.cypher(
      "MATCH ()-[r:ACTED_IN]->() WHERE r.charactername ENDS WITH 'e' RETURN r"
    )
      ...
      Bag(
        CypherMap(
          "r" -> MorpheusRelationship(
            ...
            18,
            "ACTED_IN",
            CypherMap("charactername" -> "Albus Dumbledore")
          )
        ),
        CypherMap(
          "r" -> MorpheusRelationship(
            ...
            20,
            "ACTED_IN",
            CypherMap("charactername" -> "Guenevere")
          )
        ),
        CypherMap(
          "r" -> MorpheusRelationship(
            ...
            19,
            "ACTED_IN",
            CypherMap("charactername" -> "Halle/Annie")
          )
        )
      )
    )
  ...
  it("label scan and project") {
    ...
    result.records.collect should contain(
      CypherMap("a.name" -> "Rachel Kempson")
    )
  }
  ...
  it("expand and project") {
    // When
    val result =
      graph.cypher("MATCH (a:Actor)-[r]->(m:Film) RETURN a.birthyear, m.title")

    // Then
    result.records.size shouldBe 8
    result.records.collect should contain(
      CypherMap("a.birthyear" -> 1952, "m.title" -> "Batman Begins")
    )
  }
  ...
  it("filter rels on property") {
    // Given
    val query =
      "MATCH (a:Actor)-[r:ACTED_IN]->() WHERE r.charactername = 'Guenevere' RETURN a, r"
      ...
      Bag(
        CypherMap(
          "a" -> MorpheusNode(
            2,
            Set("Actor", "Person"),
            CypherMap("birthyear" -> 1937, "name" -> "Vanessa Redgrave")
          ),
          "r" -> MorpheusRelationship(
            44,
            2,
  ...
  it("filter nodes on property") {
    // When
    val result =
      graph.cypher("MATCH (p:Person) WHERE p.birthyear = 1970 RETURN p.name")
  ...
  it("expand and project, three properties") {
    // Given
    val query =
      "MATCH (a:Actor)-[:ACTED_IN]->(f:Film) RETURN a.name, f.title, a.birthyear"
    ...
    result.records.collect should contain(
      CypherMap(
        "a.name" -> "Natasha Richardson",
        "f.title" -> "The Parent Trap",
        "a.birthyear" -> 1963
      )
    )
  }
  ...
  it("multiple hops of expand with different reltypes") {
    ...
    records.asMorpheus.df.collect().toBag should equal(
      Bag(
