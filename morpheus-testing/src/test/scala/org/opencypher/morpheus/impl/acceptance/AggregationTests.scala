...
class AggregationTests extends MorpheusTestSuite with ScanGraphInit {
  ...
  describe("AVG") {
    ...
    it("avg on durations") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [duration('P1DT12H'), duration('P1DT20H')] AS d RETURN AVG(d) AS res"
      )
  ...
  describe("COUNT") {
    ...
    it("count after expand") {
      val graph = initGraph(
        "CREATE ({name: 'foo'})-[:A]->(:B), ({name: 'bar'}), (), ()-[:A]->(:B), (), ({name: 'baz'})"
      )
    ...
    it("count() with grouping in WITH clause") {
      ...
      val result =
        graph.cypher("MATCH (n) WITH n.name AS name, count(*) AS amount RETURN name, amount")
    ...
    it("count() with grouping on multiple keys") {
      val graph = initGraph(
        "CREATE ({name: 'foo', age: 42}), ({name: 'foo', age: 42}), ({name: 'foo', age: 23}), (), (), ({name: 'baz', age: 23})"
      )

      val result = graph
        .cypher(
          "MATCH (n) WITH n.name AS name, n.age AS age, count(*) AS amount RETURN name, age, amount"
        )
  ...
  describe("MIN") {
    ...
    it("min on dates") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [date('2018-01-01'), date('2019-01-01')] AS d RETURN MIN(d) AS res"
      )
    ...
    it("min on datetimes") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [localdatetime('2010-10-10T12:00'), localdatetime('2010-10-10T12:01')] AS d RETURN MIN(d) AS res"
      )
    ...
    it("min on durations") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [duration('P1DT12H'), duration('P1DT200H')] AS d RETURN MIN(d) AS res"
      )
    ...
    it("min on combination of temporal types") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [date('2018-01-01'), localdatetime('2010-10-10T12:01')] AS d RETURN MIN(d) AS res"
      )
  ...
  describe("MAX") {
    ...
    it("max on dates") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [date('2018-01-01'), date('2019-01-01')] AS d RETURN MAX(d) AS res"
      )
    ...
    it("max on datetimes") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [localdatetime('2010-10-10T12:00'), localdatetime('2010-10-10T12:01')] AS d RETURN MAX(d) AS res"
      )
    ...
    it("max on durations") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [duration('P10DT12H'), duration('P1DT24H')] AS d RETURN MAX(d) AS res"
      )
    ...
    it("max on combination of temporal types") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [date('2018-01-01'), localdatetime('2010-10-10T12:01')] AS d RETURN MAX(d) AS res"
      )
  ...
  describe("SUM") {
    ...
    it("sum on durations") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [duration('P1DT12H'), duration('P1DT24H')] AS d RETURN SUM(d) AS res"
      )
  ...
  describe("stDev") {
    it("stDev on floats") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [98.17, 112.3, 102.6, 94.3, 108.1] AS numbers RETURN round(stDev(numbers)*1000)/1000.0 AS res"
      )
    ...
    it("stDev on nullable list of floats") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [98.17, null, 102.6, 94.3, 108.1] AS numbers RETURN round(stDev(numbers)*1000)/1000.0 AS res"
      )
  ...
  describe("stDevP") {
    it("stDevP on floats") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [98.17, 112.3, 102.6, 94.3, 108.1] AS numbers RETURN round(stDevP(numbers)*1000)/1000.0 AS res"
      )
    ...
    it("stDevP on nullable list of floats") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [98.17, null, 102.6, 94.3, 108.1] AS numbers RETURN round(stDevP(numbers)*1000)/1000.0 AS res"
      )
  ...
  describe("percentileCont") {
    it("percentileContil on integers") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [1,2] AS values RETURN percentileCont(values, 0.5) AS res"
      )
    ...
    it("percentileContil with 1.0 as percentile") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [2,10,5,6] AS values RETURN percentileCont(values, 1.0) AS res"
      )
    ...
    it("percentileContil with 0.0 as percentile") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [2,10,5,6] AS values RETURN percentileCont(values, 0.0) AS res"
      )
    ...
    it("percentileContil on floats with null") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [10.0,null,2.0,6.0] AS values RETURN round(percentileCont(values, 0.62) * 1000) / 1000.0 AS res"
      )
    ...
    it("percentileContil on floats") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [10.0,5.0,2.0,6.0] AS values RETURN percentileCont(values, 0.6) AS res"
      )
  ...
  describe("percentileDisc") {
    it("percentileDisc on integers") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [10,5,2,6] AS values RETURN percentileDisc(values, 0.5) AS res"
      )
    ...
    it("percentileDisc with 1.0 as percentile") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [10.0,5.0,2.0,6.0] AS values RETURN percentileDisc(values, 1.0) AS res"
      )
    ...
    it("percentileDisc with 0.0 as percentile") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [10.0,5.0,2.0,6.0] AS values RETURN percentileDisc(values, 0.0) AS res"
      )
    ...
    it("percentileDisc on floats with null") {
      val result = morpheus.graphs.empty.cypher(
        "UNWIND [10.0,null,2.0,6.0] AS values RETURN percentileDisc(values, 0.6) AS res"
      )
  ...
  describe("Combinations") {
    ...
    it("multiple aggregates in WITH") {
      ...
      val result = graph.cypher("""MATCH (n)
          |WITH
          | AVG(n.val) AS avg,
          | COUNT(*) AS cnt,
          | MIN(n.val) AS min,
    ...
    it("computes multiple aggregates with grouping in RETURN clause") {
      val graph =
        initGraph("CREATE ({key: 'a', val: 42}),({key: 'a',val: 23}),({key: 'b', val: 84})")
