...
class WithTests extends MorpheusTestSuite with ScanGraphInit {
  ...
  test("projecting variables in scope") {
    ...
    // When
    val result =
      given.cypher("MATCH (n:Node)-->(m:Node) WITH n, m RETURN n.val")
  ...
  test("projecting property expression") {
    ...
    // When
    val result =
      given.cypher("MATCH (n:Node)-->(m:Node) WITH n.val AS n_val RETURN n_val")
  ...
  test("projecting property expression with filter") {
    ...
    // Given
    val given = initGraph(
      """CREATE (:Node {val: 3}), (:Node {val: 4}), (:Node {val: 5})"""
    )

    // When
    val result = given.cypher(
      "MATCH (n:Node) WITH n.val AS n_val WHERE n_val <= 4 RETURN n_val"
    )
  ...
  test("projecting addition expression") {
    ...
    // When
    val result = given.cypher(
      "MATCH (n:Node)-->(m:Node) WITH n.val + m.val AS sum_n_m_val RETURN sum_n_m_val"
    )
  ...
  test("aliasing variables") {
    ...
    // When
    val result = given.cypher(
      "MATCH (n:Node)-[r]->(m:Node) WITH n.val + m.val AS sum WITH sum AS sum2 RETURN sum2"
    )
  ...
  test("projecting mixed expression") {
    ...
    // Given
    val given = initGraph(
      """CREATE (:Node {val: 4})-[:Rel]->(:Node {val: 5})-[:Rel]->(:Node)"""
    )
  ...
  test("order by") {
    val given = initGraph(
      """CREATE (:Node {val: 4}),(:Node {val: 3}),(:Node  {val: 42})"""
    )

    val result =
      given.cypher("MATCH (a) WITH a.val as val ORDER BY val RETURN val")
  ...
  test("order by asc") {
    ...
    val result =
      given.cypher("MATCH (a) WITH a.val as val ORDER BY val ASC RETURN val")
  ...
  test("order by desc") {
    ...
    val result =
      given.cypher("MATCH (a) WITH a.val as val ORDER BY val DESC RETURN val")
  ...
  test("order by with skip") {
    ...
    val result =
      given.cypher("MATCH (a) WITH a.val as val ORDER BY val SKIP 1 RETURN val")
  ...
  test("order by with (arithmetic) skip") {
    ...
    val result = given.cypher(
      "MATCH (a) WITH a.val as val ORDER BY val SKIP 1 + 1 RETURN val"
    )
  ...
  test("order by with limit") {
    ...
    val result = given.cypher(
      "MATCH (a) WITH a.val as val ORDER BY val LIMIT 1 RETURN val"
    )
  ...
  test("order by with (arithmetic) limit") {
    ...
    val result = given.cypher(
      "MATCH (a) WITH a.val as val ORDER BY val LIMIT 1 + 1 RETURN val"
    )
  ...
  test("order by with skip and limit") {
    ...
    val result = given.cypher(
      "MATCH (a) WITH a.val as val ORDER BY val SKIP 1 LIMIT 1 RETURN val"
    )
