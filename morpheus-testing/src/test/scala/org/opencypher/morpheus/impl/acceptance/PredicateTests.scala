...
class PredicateTests extends MorpheusTestSuite with ScanGraphInit {
  ...
  it("in") {
    // Given
    val given =
      initGraph("""CREATE (:A {val: 1}), (:A {val: 2}), (:A {val: 3})""")

    // When
    val result =
      given.cypher("MATCH (a:A) WHERE a.val IN [-1, 2, 5, 0] RETURN a.val")
  ...
  it("in with parameter") {
    ...
    val result = given.cypher(
      "MATCH (a:A) WHERE a.val IN $list RETURN a.val",
      Map("list" -> CypherList(-1, 2, 5, 0))
    )
  ...
  it("evaluates or") {
    ...
    // When
    val result =
      given.cypher("MATCH (a:A) WHERE a.val = 1 OR a.val = 2 RETURN a.val")
  ...
  it("or on labels") {
    // Given
    val given =
      initGraph("""CREATE (:A {val: 1}), (:B {val: 2}), (:C {val: 3})""")
  ...
  it("or on labels and properties") {
    // Given
    val given =
      initGraph("""CREATE (:A {val: 1}), (:B {val: 2}), (:A:B {val: 3})""")

    // When
    val result =
      given.cypher("MATCH (a) WHERE (a:A AND a.val = 1) OR (a:B) RETURN a.val")
  ...
  it("or with and") {
    ...
    // When
    val result = given.cypher(
      "MATCH (a:A) WHERE a.val = 1 OR (a.val >= 4 AND a.name = 'e') RETURN a.val, a.name"
    )
  ...
  it("equality between properties") {
    // Given
    val given = initGraph("""|CREATE (:A {val: 1})-[:REL]->(:B {p: 2})
    ...
    // When
    val result =
      given.cypher("MATCH (a:A)-->(b:B) WHERE a.val = b.p RETURN b.p")
  ...
  describe("comparison operators") {
    it("less than") {
      ...
      // Given
      val given =
        initGraph("""CREATE (:Node {val: 4})-[:REL]->(:Node {val: 5})""")

      // When
      val result = given.cypher(
        "MATCH (n:Node)-->(m:Node) WHERE n.val < m.val RETURN n.val"
      )
    ...
    it("compares less than between different types") {
      ...
      // Given
      val given = initGraph("""CREATE (:A {val: 4})-[:REL]->(:B {val2: 1.0}),
          |       (:A {val: 1})-[:REL]->(:B {val2: 4.0})
        """.stripMargin)

      // When
      val result =
        given.cypher("MATCH (a:A)-->(b:B) WHERE a.val < b.val2 RETURN a.val")
    ...
    it("fails when comparing less than between incompatible types") {
      ...
      // Given
      val given =
        initGraph("""CREATE (:A {val: 4})-[:REL]->(:B {val2: 'string'})""")
    ...
    it("less than or equal") {
      ...
      // When
      val result = given.cypher(
        "MATCH (n:Node)-->(m:Node) WHERE n.val <= m.val RETURN n.id, n.val"
      )
    ...
    it("compares less than or equal between different types") {
      ...
      // Given
      val given = initGraph("""CREATE (:A {val: 4})-[:REL]->(:B {val2: 4.0}),
          |       (:A {val: 1})-[:REL]->(:B {val2: 4.0})
        """.stripMargin)
  ...
  it("float conversion for integer division") {
    // Given
    val given = initGraph(
      """CREATE (:Node {id: 1, val: 4}), (:Node {id: 2, val: 5}), (:Node {id: 3, val: 5})"""
    )

    // When
    val result = given.cypher(
      "MATCH (n:Node) WHERE (n.val * 1.0) / n.id >= 2.5 RETURN n.id"
    )
  ...
  it("basic pattern predicate") {
    ...
    // When
    val result =
      given.cypher("MATCH (a)-->(b) WHERE (a)-->()-->(b) RETURN a.id, b.id")
  ...
  it("pattern predicate with var-length-expand") {
    ...
    // When
    val result = given.cypher(
      "MATCH (a)-->(b) WHERE (a)-[*1..3]->()-->(b) RETURN a.id, b.id"
    )
  ...
  it("simple pattern predicate with node predicate") {
    ...
    // When
    val result =
      given.cypher("MATCH (a) WHERE (a)-->({name: 'foo'}) RETURN a.id")
  ...
  it("simple pattern predicate with relationship predicate") {
    ...
    // When
    val result = given.cypher(
      "MATCH (a)-->(b) WHERE (a)-[{val: 'foo'}]->()-->(b) RETURN a.id, b.id"
    )
  ...
  it("simple pattern predicate with node label predicate") {
    ...
    // When
    val result =
      given.cypher("MATCH (a)-->(b) WHERE (a)-->(:A)-->(b) RETURN a.id, b.id")
  ...
  it("inverse pattern predicate") {
    ...
    // When
    val result =
      given.cypher("MATCH (a)-->(b) WHERE NOT (a)-->()-->(b) RETURN a.id, b.id")
  ...
  it("pattern predicate with derived node predicate") {
    ...
    // When
    val result =
      given.cypher("MATCH (a) WHERE (a)-->({val: a.val + 2}) RETURN a.id")
  ...
  it("multiple predicate patterns") {
    ...
    // When
    val result =
      given.cypher("MATCH (a) WHERE (a)-->({id: 2, foo: true}) RETURN a.id")
  ...
  describe("Inline pattern predicates") {
    ...
    it("simple pattern predicate with relationship type predicate") {
      ...
      // When
      val result = given.cypher(
        "MATCH (a)-->(b) WHERE (a)-[:A]->()-->(b) RETURN a.id, b.id"
      )
    ...
    it("inverse pattern predicate") {
      ...
      // When
      val result = given.cypher(
        "MATCH (a)-->(b) WHERE NOT (a)-->()-->(b) RETURN a.id, b.id"
      )
  ...
  describe("Pattern predicates via exists") {
    it("basic pattern predicate") {
      ...
      // When
      val result = given.cypher(
        "MATCH (a)-->(b) WHERE EXISTS((a)-->()-->(b)) RETURN a.id, b.id"
      )
    ...
    it("pattern predicate with var-length-expand") {
      ...
      // When
      val result = given.cypher(
        "MATCH (a)-->(b) WHERE EXISTS((a)-[*1..3]->()-->(b)) RETURN a.id, b.id"
      )
    ...
    it("simple pattern predicate with node predicate") {
      ...
      // When
      val result = given.cypher(
        "MATCH (a) WHERE EXISTS((a)-->({name: 'foo'})) RETURN a.id"
      )
    ...
    it("simple pattern predicate with relationship predicate") {
      ...
      // When
      val result = given.cypher(
        "MATCH (a)-->(b) WHERE EXISTS((a)-[{val: 'foo'}]->()-->(b)) RETURN a.id, b.id"
      )
    ...
    it("simple pattern predicate with node label predicate") {
      ...
      // When
      val result = given.cypher(
        "MATCH (a)-->(b) WHERE EXISTS((a)-->(:A)-->(b)) RETURN a.id, b.id"
      )
    ...
    it("inverse pattern predicate") {
      ...
      // When
      val result = given.cypher(
        "MATCH (a)-->(b) WHERE NOT EXISTS((a)-->()-->(b)) RETURN a.id, b.id"
      )
    ...
    it("pattern predicate with derived node predicate") {
      ...
      // When
      val result = given.cypher(
        "MATCH (a) WHERE EXISTS((a)-->({val: a.val + 2})) RETURN a.id"
      )
    ...
    it("multiple predicate patterns") {
      ...
      // When
      val result = given.cypher(
        "MATCH (a) WHERE EXISTS((a)-->({id: 2, foo: true})) RETURN a.id"
      )
