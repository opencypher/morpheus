...
class MultipleGraphTests extends MorpheusTestSuite with ScanGraphInit {
  ...
  def testGraph1: RelationalCypherGraph[SparkTable.DataFrameTable] = initGraph(
    "CREATE (:Person {name: 'Mats'})"
  )

  def testGraph2: RelationalCypherGraph[SparkTable.DataFrameTable] = initGraph(
    "CREATE (:Person {name: 'Phil'})"
  )

  def testGraph3: RelationalCypherGraph[SparkTable.DataFrameTable] = initGraph(
    "CREATE (:Car {type: 'Toyota'})"
  )

  def testGraphRels: RelationalCypherGraph[SparkTable.DataFrameTable] =
    initGraph("""|CREATE (mats:Person {name: 'Mats'})
       |CREATE (max:Person {name: 'Max'})
       |CREATE (max)-[:HAS_SIMILAR_NAME]->(mats)
    """.stripMargin)

  it("can read graph via parameter") {
    val graph = initGraph("""|CREATE (:A {v: 1})
  ...
  it("constructs from different graphs with multiple distinct nodes") {
    val g1 = initGraph("""|CREATE (:A {v: 1})
         |CREATE (:B {v: 100})""".stripMargin)
    val g2 = initGraph("""|CREATE (:A {v: 2})
  ...
  it(
    "should CONSTRUCT a graph with multiple unconnected anonymous CREATE clauses"
  ) {
    val query =
      """|CONSTRUCT
         |  CREATE (:A)
  ...
  it("should construct a node property from a matched node") {
    ...
    result.graph.schema should equal(
      PropertyGraphSchema.empty.withNodePropertyKeys("A")("name" -> CTString)
    )
    result.graph
  ...
  it("should construct a node property from a literal") {
    ...
    result.getRecords shouldBe None
    result.graph.schema should equal(
      PropertyGraphSchema.empty.withNodePropertyKeys()("name" -> CTString)
    )
    result.graph
      .cypher("MATCH (a) RETURN a.name")
      .records
      .iterator
      .toBag should equal(
      Bag(
    ...
    result.graph.schema should equal(
      PropertyGraphSchema.empty
        .withNodePropertyKeys(
          Set("A", "B"),
          PropertyKeys("name" -> CTString, "age" -> CTInteger)
        )
    )
    result.graph
      .cypher("MATCH (a:A:B) RETURN a.name")
      .records
      .iterator
      .toBag should equal(
      Bag(
  ...
  it("should copy a relationship") {
    ...
    result.getRecords shouldBe None
    result.graph
      .cypher("MATCH ()-[r]->() RETURN r.val, r.name, type(r) as type")
      .records
      .iterator
      .toBag should equal(
      Bag(
  ...
  it("should copy a mean relationship") {
    ...
    result.graph
      .cypher("MATCH ()-[r]->() RETURN r.val, r.val2, type(r) as type")
      .records
      .iterator
      .toBag should equal(
      Bag(
  ...
  it("should copy a node") {
    ...
    result.graph
      .cypher("MATCH (a) RETURN a.foo, labels(a) as labels")
      .records
      .iterator
      .toBag should equal(
      Bag(
  ...
  it("should copy a node with labels") {
    ...
    result.getRecords shouldBe None
    result.graph
      .cypher("MATCH (a) RETURN a.val, labels(a) as labels")
      .records
      .iterator
      .toBag should equal(
      Bag(
  ...
  it("CONSTRUCTS ON a single graph") {
    ...
    result.relationships("r").toMaps should equal(
      testGraph1.relationships("r").toMaps
    )
  }
  ...
  it("CONSTRUCT ON a single graph without GraphUnionAll") {
    ...
    result.asMorpheus.maybeRelational match {
      case Some(relPlan) =>
        val switchOp = relPlan.collectFirst { case op: SwitchContext[_] =>
          op
        }.get
        val containsUnionGraph =
          switchOp.context.queryLocalCatalog.head._2 match {
            case g: UnionGraph[_] =>
              g.graphs.collectFirst { case op: UnionGraph[_] => op }.isDefined
            case _ => false
          }
  ...
  it("CONSTRUCTS ON two graphs") {
    ...
    result.nodes("n").toMaps should equal(
      testGraph1.unionAll(testGraph2).nodes("n").toMaps
    )
    result.relationships("r").toMaps should equal(
      testGraph1.unionAll(testGraph2).relationships("r").toMaps
    )
  }
  ...
  it("CONSTRUCTS ON two graphs and adds a relationship") {
    ...
    result.schema should equal(
      (testGraph1.schema ++ testGraph2.schema).withRelationshipPropertyKeys(
        "KNOWS"
      )()
    )
    result.nodes("n").toMaps should equal(
    ...
    val resultRelationship = result
      .relationships("r")
      .toMaps
      .head
      ._1("r")
      .asInstanceOf[MorpheusRelationship]
  ...
  it(
    "implictly clones when CONSTRUCTing ON two graphs and adding a relationship"
  ) {
    morpheus.catalog.store("one", testGraph1)
    morpheus.catalog.store("two", testGraph2)
    val query =
  ...
  it("constructs a created node") {
    ...
    graph.schema should equal(
      PropertyGraphSchema.empty.withNodePropertyKeys(Set.empty[String])
    )
  ...
  it(
    "does not clone twice when a variable is both constructed on and matched"
  ) {
  ...
  it("allows cloning from different graphs with nodes and relationships") {
    def testGraphRels = initGraph("""|CREATE (mats:Person {name: 'Mats'})
         |CREATE (max:Person {name: 'Max'})
         |CREATE (max)-[:HAS_SIMILAR_NAME]->(mats)
      """.stripMargin)
  ...
  it("can construct a copy of a node with matched label") {
    morpheus.cypher(
      "CATALOG CREATE GRAPH foo { CONSTRUCT CREATE (:A) RETURN GRAPH }"
    )
  ...
  it("can construct with an input table expanded by unwind") {
    ...
    val data = morpheus
      .cypher("FROM GRAPH foo RETURN GRAPH")
      .graph
      .cypher("""MATCH (a:A)
        |UNWIND [1, 2, 3] AS i
        |CONSTRUCT
    ...
    data.iterator.toBag should equal(
      Bag(
        nullRow,
        nullRow,
        nullRow,
  ...
  it("fails early on incompatible graphs in CONSTRUCT") {
    ...
    val g2 =
      morpheus.cypher("CONSTRUCT CREATE (:A {p: 'foo'}) RETURN GRAPH").graph
    morpheus.catalog.store("g1", g1)
    morpheus.catalog.store("g2", g2)

    an[SchemaException] should be thrownBy morpheus.cypher(
      "CONSTRUCT ON g1, g2 RETURN GRAPH"
    )
  }
  ...
  it("should implicit clone a relationship #1") {
    ...
    result.graph
      .cypher("MATCH ()-[r]->() RETURN type(r) as type")
      .records
      .iterator
      .toBag should equal(
      Bag(
