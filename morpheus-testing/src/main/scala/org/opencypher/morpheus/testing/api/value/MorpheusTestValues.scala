 ...
 */
package org.opencypher.morpheus.testing.api.value
...
object MorpheusTestValues {
  ...
  implicit lazy val NODE_valueGroups: ValueGroups[CypherValue] = Seq(
    Seq[CypherValue](
      MorpheusNode(1, Set("Person"), CypherMap("a" -> 1, "b" -> CypherNull)),
      MorpheusNode(1, Set("Person"), CypherMap("a" -> 1.0d, "b" -> CypherNull))
    ),
