...
trait ElementTableCreationSupport {
  ...
  /**
    * This helper creates an ElementTable based on column name conventions. For every pattern
    * element with name NAME, the following column names are recognized:
    *
    *   - NAME_id / NAME_source / NAME_target -> recognized as id, source or target columns, eg.
    *     *node_source*
    *   - NAME_PROPERTY_property -> as a property column with property name PROPERTY, eg.
    *     *node_name_property*
    *
  ...
  def constructElementTable(
    ...
    val mapping = pattern.elements.foldLeft(ElementMapping.empty(pattern)) {
      case (acc, patternElement) =>
        val patternElementColumns = df.columns
        ...
        val propertyMapping: Map[String, String] =
          patternElementColumns.collect {
            case prop if prop.endsWith("_property") =>
              val encodedKey = prop
                .replaceFirst(s"${patternElement.name}_", "")
                .replaceFirst("_property", "")
