...
class SchemaFromProcedureTest extends FunSuite with BeforeAndAfter with Matchers {
  ...
  test("single label") {
    ...
    schema should equal(Schema.empty
      .withNodePropertyKeys("A")("val1" -> CTString, "val2" -> CTNumber)
    )
  }

  test("input order does not affect schema") {
    val schema = schemaFor(
      """|CREATE (b1:B { type: 'B1' })
  ...
  test("single and multiple labels") {
    val schema = schemaFor(
  ...
  test("single relationship") {
    ...
    schema should equal(Schema.empty
      .withNodePropertyKeys("A")()
      .withRelationshipPropertyKeys("REL")("val1" -> CTString, "val2" -> CTAny)
    )
  }
  ...
  test("unsupported data types") {
    val createQuery =
      """|CREATE (a:A { foo: time(), bar : 42 })
         |CREATE (b:A)
         |CREATE (a)-[:REL]->(b)""".stripMargin
    a[UnsupportedOperationException] should be thrownBy schemaFor(createQuery)
    ...
    schemaWithOmittedImportFailures should equal(Schema.empty
      .withNodePropertyKeys("A")("foo" -> CTVoid, "bar" -> CTInteger.nullable)
      .withRelationshipPropertyKeys("REL")()
    )
  }
