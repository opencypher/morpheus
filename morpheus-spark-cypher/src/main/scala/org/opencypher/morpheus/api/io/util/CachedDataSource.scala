...
/**
  * Wraps a [[PropertyGraphDataSource]] and introduces a caching mechanism. First time a graph is
  * read, it is cached in Spark according to the given [[StorageLevel]] and put in the datasource
  * internal cache. When a graph is removed from the data source, it is uncached in Spark and
  * removed from the datasource internal cache.
  *
  * @param dataSource
...
case class CachedDataSource(
  ...
  override def graph(name: GraphName): PropertyGraph = cache.getOrElse(
    name, {
  ...
  override def hasGraph(name: GraphName): Boolean =
    cache.contains(name) || dataSource.hasGraph(name)

  override def schema(name: GraphName): Option[PropertyGraphSchema] =
    dataSource.schema(name)

  override def store(name: GraphName, graph: PropertyGraph): Unit =
    dataSource.store(name, graph)
