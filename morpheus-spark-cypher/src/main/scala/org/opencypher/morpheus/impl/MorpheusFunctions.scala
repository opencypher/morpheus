...
object MorpheusFunctions {
  ...
  @nowarn
  val EMPTY_STRUCT: Column = udf(
    () => new GenericRowWithSchema(Array(), StructType(Nil)),
    StructType(Nil)
  )()
  ...
  implicit class RichColumn(column: Column) {
    ...
    /**
      * This is a copy of {{{org.apache.spark.sql.Column#getItem}}}. The original method only allows
      * fixed values (Int, or String) as index although the underlying implementation seem capable
      * of processing arbitrary expressions. This method exposes these features
      */
    def get(idx: Column): Column =
      new Column(UnresolvedExtractValue(column.expr, idx.expr))
  }

  private[morpheus] val rowIdSpaceBitsUsedByMonotonicallyIncreasingId = 33

  /**
    * Configurable wrapper around `monotonically_increasing_id`
    *
    * @param partitionStartDelta
    *   Conceptually this number is added to the `partitionIndex` from which the Spark function
  ...
  def list_slice(
    list: Column,
    maybeFrom: Option[Column],
    maybeTo: Option[Column]
  ): Column = {
    val start = maybeFrom.map(_ + ONE_LIT).getOrElse(ONE_LIT)
    val length = If(
      (size(list) === ZERO_LIT).expr,
      NULL_LIT.expr,
      ((maybeTo.getOrElse(size(list)) - start) + ONE_LIT).expr
    )
  ...
  def regex_match(text: Column, pattern: Column): Column = new Column(
    RLike(text.expr, pattern.expr)
  )
  ...
  private val x: NamedLambdaVariable = NamedLambdaVariable(
    "x",
    StructType(
      Seq(StructField("item", StringType), StructField("flag", BooleanType))
    ),
    nullable = false
  )
  ...
  def filter_not_null[T: TypeTag](items: Seq[T], mask: Seq[Column]): Column = {
    filter_with_mask(
      items,
      mask,
      LambdaFunction(IsNotNull(GetStructField(x, 1)), Seq(x), hidden = false)
    )
  }

  def make_big_decimal(
    unscaledVal: Column,
    precision: Int,
    scale: Int
  ): Column = {
  ...
  private def filter_with_mask[T: TypeTag](
    items: Seq[T],
    mask: Seq[Column],
    predicate: LambdaFunction
  ): Column = {
    require(
      items.size == mask.size,
      s"Array filtering requires for the items and the mask to have the same length."
    )
    if (items.isEmpty) {
      functions.array()
    } else {
      ...
      val zippedArray =
        functions.arrays_zip(itemLiterals, functions.array(mask: _*))
      val filtered = ArrayFilter(zippedArray.expr, predicate)
      val transform = ArrayTransform(
        filtered,
        LambdaFunction(GetStructField(x, 0), Seq(x), hidden = false)
      )
  ...
  def switch(
    branches: Seq[(Column, Column)],
    maybeDefault: Option[Column]
  ): Column = {
    new Column(
      CaseWhen(
        branches.map { case (c, v) => c.expr -> v.expr },
        maybeDefault.map(_.expr)
      )
    )
  }

  /**
    * Alternative version of {{{org.apache.spark.sql.functions.translate}}} that takes
    * {{{org.apache.spark.sql.Column}}} s for search and replace strings.
    */
  def translate(
    src: Column,
    matchingString: Column,
    replaceString: Column
  ): Column = {
    new Column(
      StringTranslate(src.expr, matchingString.expr, replaceString.expr)
    )
  }

  def column_for(
    expr: Expr
  )(implicit header: RecordHeader, df: DataFrame): Column = {
    val columnName = header
      .getColumn(expr)
      .getOrElse(
