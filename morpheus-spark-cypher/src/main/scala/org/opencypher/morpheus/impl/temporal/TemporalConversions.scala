...
object TemporalConversions extends Logging {
  ...
  implicit class RichDuration(duration: Duration) {
    ...
    /**
      ...
      *
      * @note
      *   This conversion is lossy, as the Sparks [[CalendarInterval]] only has a resolution down to
      *   microseconds. Additionally it uses an approximate representation of days.
      */
    def toCalendarInterval: CalendarInterval = {
      if (duration.nanos % 1000 != 0) {
        logger.warn(
          "Spark does not support durations with nanosecond resolution, truncating!"
        )
      }
  ...
  /**
    ...
    *
    * @note
    *   To ensure compatibility with the reverse operation we estimate the number of days from the
    *   given seconds.
    */
  implicit class RichCalendarInterval(calendarInterval: CalendarInterval) {
    def toDuration: Duration = {
      val daysInSeconds =
        calendarInterval.days * DateTimeConstants.SECONDS_PER_DAY
      val seconds =
        daysInSeconds + (calendarInterval.microseconds / DateTimeConstants.MICROS_PER_SECOND)
      val normalizedDays =
        seconds / (DateTimeConstants.MICROS_PER_DAY / DateTimeConstants.MICROS_PER_SECOND)
      val normalizedSeconds =
        seconds % (DateTimeConstants.MICROS_PER_DAY / DateTimeConstants.MICROS_PER_SECOND)
      val normalizedNanos =
        calendarInterval.microseconds % DateTimeConstants.MICROS_PER_SECOND * 1000
  ...
  implicit class TemporalExpression(val expr: Expr) extends AnyVal {
        ...
        .map {
          ...
          case _ =>
            throw IllegalStateException(
              "Spark does not support nanosecond resolution in 'localdatetime'"
            )
        }
        .orNull
    }
    ...
    def resolveTemporalArgument(implicit
      parameters: CypherMap
    ): Option[Either[Map[String, Int], String]] = {
      expr match {
        case MapExpression(inner) =>
          val map = inner.map {
            case (key, Param(name)) =>
              key -> (parameters(name) match {
                case CypherString(s)  => s.toInt
                case CypherInteger(i) => i.toInt
                case other =>
                  throw IllegalArgumentException(
                    "A map value of type CypherString or CypherInteger",
                    other
                  )
              })
            case (key, e) =>
              throw NotImplementedException(
                s"Parsing temporal values is currently only supported for Literal-Maps, got $key -> $e"
              )
          }
        ...
        case Param(name) =>
          val s = parameters(name) match {
            ...
            case other =>
              throw IllegalArgumentException(
                s"Parameter `$name` to be a CypherString",
                other
              )
          }
        ...
        case other =>
          throw NotImplementedException(
            s"Parsing temporal values is currently only supported for Literal-Maps and String literals, got $other"
          )
      }
    }
  ...
  def temporalAccessor[I: TypeTag](
    ...
    accessor.toLowerCase match {
      ...
      case "dayofweek" | "weekday" =>
        TemporalUdfs.dayOfWeek[I].apply(temporalColumn)
      ...
      case other =>
        throw UnsupportedOperationException(
          s"Unknown Temporal Accessor: $other"
        )
    }
  }
}
