...
object TemporalUdafs extends Logging {
  ...
  trait SimpleDurationAggregation
      extends Aggregator[CalendarInterval, CalendarInterval, CalendarInterval] {
    final override def finish(reduction: CalendarInterval): CalendarInterval =
      reduction
    final override def bufferEncoder: Encoder[CalendarInterval] =
      intervalEncoder
    final override def outputEncoder: Encoder[CalendarInterval] =
      intervalEncoder
  }
  ...
  object DurationSum extends SimpleDurationAggregation {
    ...
    override def reduce(
      b: CalendarInterval,
      a: CalendarInterval
    ): CalendarInterval = IntervalUtils.add(b, a)
    override def merge(
      b1: CalendarInterval,
      b2: CalendarInterval
    ): CalendarInterval = IntervalUtils.add(b1, b2)
  }
  ...
  object DurationMax extends SimpleDurationAggregation {
    ...
    override def merge(
      b1: CalendarInterval,
      b2: CalendarInterval
    ): CalendarInterval = reduce(b1, b2)
  }

  object DurationMin extends SimpleDurationAggregation {
    final override def zero: CalendarInterval =
  ...
  case class DurationAvgRunningSum(
    months: Int,
    days: Int,
    micros: Long,
    count: Long
  ...
  object DurationAvg
    ...
    override def reduce(
      b: DurationAvgRunningSum,
      a: CalendarInterval
    ): DurationAvgRunningSum = DurationAvgRunningSum(
    ...
    override def merge(
      b1: DurationAvgRunningSum,
      b2: DurationAvgRunningSum
    ): DurationAvgRunningSum = {
      DurationAvgRunningSum(
    ...
    override def bufferEncoder: Encoder[DurationAvgRunningSum] =
      Encoders.product[DurationAvgRunningSum]
