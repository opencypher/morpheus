...
object SparkSQLExprMapper {
  ...
  implicit class RichExpression(expr: Expr) {
    ...
    /**
      * Converts `expr` with the `withConvertedChildren` function, which is passed the converted
      * child expressions as its argument.
      *
      * Iff the expression has `expr.nullInNullOut == true`, then any child being mapped to `null`
      * will also result in the parent expression being mapped to null.
      *
      * For these expressions the `withConvertedChildren` function is guaranteed to not receive any
      * `null` values from the evaluated children.
      */
    def nullSafeConversion(
      expr: Expr
    )(withConvertedChildren: Seq[Column] => Column)(implicit
      header: RecordHeader,
      df: DataFrame,
      ...
      if (expr.cypherType == CTNull) {
        ...
        val withConvertedChildrenResult = withConvertedChildren(
          evaluatedArgs
        ).expr
        if (expr.children.nonEmpty && expr.nullInNullOut && expr.cypherType.isNullable) {
          val nullPropagationCases = evaluatedArgs
            .map(_.isNull.expr)
            .zip(Seq.fill(evaluatedArgs.length)(NULL_LIT.expr))
          new Column(
            CaseWhen(nullPropagationCases, withConvertedChildrenResult)
          )
        } else {
    ...
    /**
      ...
      *
      * @param header
      *   the header of the [[MorpheusRecords]] in which the expression should be evaluated.
      * @param df
      *   the dataframe containing the data over which the expression should be evaluated.
      * @param parameters
      *   query parameters
      * @return
      *   Some Spark SQL expression if the input was mappable, otherwise None.
      */
    def asSparkSQLExpr(implicit
      header: RecordHeader,
      df: DataFrame,
      parameters: CypherMap
    ): Column = {
      val outCol = expr match {
        case v: LambdaVar =>
          val sparkType = v.cypherType.toSparkType.getOrElse(
            throw IllegalStateException(s"No valid dataType for LambdaVar $v")
          )
          new Column(
            NamedLambdaVariable(
              v.name,
              sparkType,
              nullable = v.cypherType.isNullable,
              ExprId(v.hashCode.toLong)
            )
          )
    ...
    private def convert(convertedChildren: Seq[Column])(implicit
      header: RecordHeader,
      df: DataFrame,
      parameters: CypherMap
    ): Column = {
      ...
      expr match {
        ...
        case _: ElementProperty =>
          if (!header.contains(expr)) NULL_LIT else column_for(expr)
        case MapProperty(_, key) =>
          if (expr.cypherType.material == CTVoid) NULL_LIT
          else child0.getField(key.name)
        case DateProperty(_, key) =>
          temporalAccessor[java.sql.Date](child0, key.name)
        case LocalDateTimeProperty(_, key) =>
          temporalAccessor[java.sql.Timestamp](child0, key.name)
        case DurationProperty(_, key) =>
          TemporalUdfs.durationAccessor(key.name.toLowerCase).apply(child0)

        case LocalDateTime(maybeDateExpr) =>
          maybeDateExpr
            .map(e => lit(e.resolveTimestamp).cast(DataTypes.TimestampType))
            .getOrElse(current_timestamp())
        case Date(maybeDateExpr) =>
          maybeDateExpr
            .map(e => lit(e.resolveDate).cast(DataTypes.DateType))
            .getOrElse(current_timestamp())
        case Duration(durationExpr) => lit(durationExpr.resolveInterval)

        case In(lhs, rhs) =>
          rhs.cypherType.material match {
            case CTList(inner) if inner.couldBeSameTypeAs(lhs.cypherType) =>
              array_contains(child1, child0)
            case _ => NULL_LIT
          }
            ...
            case (CTList(lhInner), CTList(rhInner)) =>
              if ((lhInner | rhInner).isSparkCompatible) {
                ...
                throw SparkSQLMappingException(
                  s"Lists of different inner types are not supported (${lhInner.material}, ${rhInner.material})"
                )
              }
            case (CTList(inner), nonListType) if (inner | nonListType).isSparkCompatible =>
              concat(child0, array(child1))
            case (nonListType, CTList(inner)) if (inner | nonListType).isSparkCompatible =>
              concat(array(child0), child1)
            case (CTString, _) if rhsCT.subTypeOf(CTNumber) =>
              concat(child0, child1.cast(StringType))
            case (_, CTString) if lhsCT.subTypeOf(CTNumber) =>
              concat(child0.cast(StringType), child1)
        ...
        case Subtract(lhs, rhs)
            if lhs.cypherType.material.subTypeOf(
              CTDate
            ) && rhs.cypherType.material.subTypeOf(CTDuration) =>
          ...
          e.cypherType.material match {
            ...
            case other =>
              throw IllegalArgumentException(
                "a type that may be converted to an ID",
                other
              )
          }
        ...
        case Labels(e) =>
          val possibleLabels =
            header.labelsFor(e.owner.get).toSeq.sortBy(_.label.name)
          val labelBooleanFlagsCol = possibleLabels.map(_.asSparkSQLExpr)
          val nodeLabels =
            filter_true(possibleLabels.map(_.label.name), labelBooleanFlagsCol)
          nodeLabels

        case Type(e) =>
          val possibleRelTypes =
            header.typesFor(e.owner.get).toSeq.sortBy(_.relType.name)
          val relTypeBooleanFlagsCol = possibleRelTypes.map(_.asSparkSQLExpr)
          val relTypes = filter_true(
            possibleRelTypes.map(_.relType.name),
            relTypeBooleanFlagsCol
          )
          ...
          e.cypherType.material match {
            case element if element.subTypeOf(CTElement) =>
              val possibleProperties =
                header.propertiesFor(e.owner.get).toSeq.sortBy(_.key.name)
            ...
            case CTMap(inner) =>
              ...
              val (propertyKeys, propertyValues) = inner.keys
                .map { e =>
                  // Whe have to make sure that every column has the same type (true or null)
                  e -> when(mapColumn.getField(e).isNotNull, TRUE_LIT)
                    .otherwise(NULL_LIT)
                }
                .toSeq
                .unzip
            ...
            case other =>
              throw IllegalArgumentException(
                "an Expression with type CTNode, CTRelationship or CTMap",
                other
              )
          }
          ...
          e.cypherType.material match {
            case element if element.subTypeOf(CTElement) =>
              ...
              val propertyColumns = propertyExpressions
                .map(propertyExpression =>
                  propertyExpression.asSparkSQLExpr.as(
                    propertyExpression.key.name
                  )
                )
            ...
            case other =>
              throw IllegalArgumentException(
                "a node, relationship or map",
                other,
                "Invalid input to properties function"
              )
          }

        case StartNodeFunction(e) =>
          header.startNodeFor(e.owner.get).asSparkSQLExpr
        ...
        case _: Range =>
          sequence(child0, child1, convertedChildren.lift(2).getOrElse(ONE_LIT))
        case _: Replace => translate(child0, child1, child2)
        case _: Substring =>
          child0.substr(
            child1 + ONE_LIT,
            convertedChildren.lift(2).getOrElse(length(child0) - child1)
          )
        case _: Split =>
          new Column(StringSplit(child0.expr, child1.expr, lit(-1).expr))
        ...
        case Haversin(e) =>
          Divide(Subtract(IntegerLit(1), Cos(e)), IntegerLit(2)).asSparkSQLExpr
        ...
        case ShiftLeft(_, IntegerLit(shiftBits)) =>
          shiftleft(child0, shiftBits.toInt)
        case ShiftRightUnsigned(_, IntegerLit(shiftBits)) =>
          shiftrightunsigned(child0, shiftBits.toInt)
        ...
        case CaseExpr(_, maybeDefault) =>
          val (maybeConvertedDefault, convertedAlternatives) =
            if (maybeDefault.isDefined) {
        ...
        case ContainerIndex(container, index) =>
          ...
          container.cypherType.material match {
            case c if c.subTypeOf(CTContainer) =>
              containerCol.get(index.asSparkSQLExpr)
            case other =>
              throw NotImplementedException(
                s"Accessing $other by index is not supported"
              )
          }

        case _: ListSliceFromTo =>
          list_slice(child0, Some(child1), Some(child2))
        ...
        case ListComprehension(
          ...
          val lambdaVar = variable.asSparkSQLExpr.expr match {
            ...
            case err =>
              throw IllegalStateException(
                s"$variable should be converted into a NamedLambdaVariable instead of $err"
              )
          }
          val filteredExpr = innerPredicate match {
            case Some(filterExpr) =>
              val filterFunc =
                LambdaFunction(filterExpr.asSparkSQLExpr.expr, Seq(lambdaVar))
          ...
          val result = extractExpression match {
            case Some(extractExpr) =>
              val extractFunc =
                LambdaFunction(extractExpr.asSparkSQLExpr.expr, Seq(lambdaVar))
        ...
        case ListReduction(acc, v, _, _, _) =>
          ...
          val (initVar, accVar) = convertedChildrenExpr.slice(0, 2) match {
            ...
            case err =>
              throw IllegalStateException(
                s"($v,$acc) should be converted into a NamedLambdaVariables instead of $err"
              )
          }
          val reduceFunc =
            LambdaFunction(convertedChildrenExpr(2), Seq(initVar, accVar))
          val finishFunc = LambdaFunction(accVar, Seq(accVar))

          val reduceExpr = ArrayAggregate(
            convertedChildrenExpr(4),
            convertedChildrenExpr(3),
            reduceFunc,
            finishFunc
          )
        ...
        case predExpr: IterablePredicateExpr =>
          ...
          val lambdaVar = child0.expr match {
            ...
            case err =>
              throw IllegalStateException(
                s"${predExpr.exprs.head} should be converted into a NamedLambdaVariable instead of $err"
              )
          }
          val filterFunc =
            LambdaFunction(convertedChildrenExpr(1), Seq(lambdaVar))
          predExpr match {
            case _: ListAll =>
        ...
        case MapExpression(items) =>
          expr.cypherType.material match {
            case CTMap(_) =>
              val innerColumns = items.map { case (key, innerExpr) =>
                innerExpr.asSparkSQLExpr.as(key)
              }.toSeq
              create_struct(innerColumns)
            case other =>
              throw IllegalArgumentException(
                "an expression of type CTMap",
                other
              )
          }

        case MapProjection(mapOwner, items, includeAllProps) =>
          val convertedItems = items.map { case (key, value) =>
            value.asSparkSQLExpr.as(key)
          }
          ...
          val intersectedMapItems = if (includeAllProps) {
            mapOwner.cypherType.material match {
              case x if x.subTypeOf(CTElement) =>
                val uniqueEntityProps = header
                  .propertiesFor(mapOwner)
                  .filterNot(p => itemKeys.contains(p.key.name))
                val propertyColumns =
                  uniqueEntityProps.map(p => p.asSparkSQLExpr.as(p.key.name))
                convertedItems ++ propertyColumns
              case CTMap(inner) =>
                val uniqueMapKeys =
                  inner.keys.filterNot(key => itemKeys.contains(key))
                val uniqueMapColumns =
                  uniqueMapKeys.map(key => child0.getField(key).as(key))
        ...
        case _: PercentileCont =>
          val percentile = child1.expr match {
            ...
            case _ =>
              throw IllegalArgumentException(
                "Literal as percentage for percentileCont()",
                child1.expr
              )
          }
        ...
        case _: PercentileDisc =>
          val percentile = child1.expr match {
            ...
            case _ =>
              throw IllegalArgumentException(
                "Literal as percentage for percentileDisc()",
                child1.expr
              )
          }
          PercentileUdafs.percentileDisc(percentile, child0.expr.dataType)(
            child0
          )
        ...
        case _ =>
          throw NotImplementedException(
            s"No support for converting Cypher expression $expr to a Spark SQL expression"
          )
      }
    }
