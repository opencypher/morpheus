...
import org.opencypher.caps.test.fixture.{
  MiniDFSClusterFixture,
  Neo4jServerFixture,
  SparkSessionFixture
}
...
class GCDemoTest
  ...
  ignore("the demo") {
    ...
    val CITYFRIENDS_EU = SN_EU.cypher(
      """MATCH (a:Person)-[:LIVES_IN]->(city:City)<-[:LIVES_IN]-(b:Person), (a)-[:KNOWS*1..2]->(b)
        |WHERE city.name = "Malm?" OR city.name = "Berlin"
        |RETURN GRAPH result OF (a)-[r:ACQUAINTED]->(b)
      """.stripMargin)
    ...
    val ALL_CITYFRIENDS =
      (CITYFRIENDS_EU.graphs("result") union CITYFRIENDS_US.graphs("result")).persist(storageLevel)
    ...
    val RECO =
      (ALL_CITYFRIENDS union PRODUCTS union LINKS union SN_EU union SN_US).persist(storageLevel)

    check(verifyReco(RECO))

    val result =
      RECO.cypher("""MATCH (a:Person)-[:ACQUAINTED]-(b:Person)-[:HAS_INTEREST]->(i:Interest),
  ...
  test("write back to Neo") {
    ...
    withBoltSession { session =>
      result.records.toLocalScalaIterator.foreach { cypherMap =>
        session.run(
          s"MATCH (p:Person {name: ${cypherMap.get("name").get}}) SET p.should_buy = 'a book'")
      }
    }
    ...
    res.records.toLocalScalaIterator.toSet should equal(
      Set(
  ...
  private def withBoltSession[T](f: Session => T): T = {
    val driver = org.neo4j.driver.v1.GraphDatabase
      .driver(neo4jHost,
              AuthTokens.basic(neo4jConfig.user, neo4jConfig.password.get),
              neo4jConfig.boltConfig())
  ...
  def verifyRecoResult(r: CAPSResult) = {
    ...
    r.records.toCypherMaps.collect().toSet should equal(
      Set(
  ...
  def verifyReco(graph: CAPSGraph) = {
    ...
    graph.schema should equal(
      Schema.empty
        ...
        .withNodePropertyKeys("Product")("title"    -> CTString.nullable,
        ...
        .withRelationshipPropertyKeys("BOUGHT")("rating"  -> CTInteger.nullable,
    ...
    Bag(graph.nodes("n").toDF().collect(): _*) should equal(
      Bag(
        ...
        Row(3L, true, false, false, false, false, "Malm?", null, "EU", null, null),
    ...
    Bag(graph.relationships("r").toDF().drop("r").collect(): _*) should equal(
      Bag(
...
//    println(baseTable.header)
  ...
  def verifyCityFriendsUS(g: CAPSGraph): Assertion = {
    ...
    Bag(g.nodes("n").toDF().collect(): _*) should equal(
      Bag(
  ...
  override def dataFixture =
    """
       ...
       CREATE (mal:City {name: "Malm?", region: "EU"})
