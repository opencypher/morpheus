...
class FlatPlannerTest extends BaseTestSuite {
  ...
  test("flat plan for expand") {
    val result = flatPlanner.process(
      mkLogical.planSourceExpand(IRField("n")(CTNode),
  ...
  test("flat plan for expand with rel type info") {
    val result = flatPlanner.process(
      mkLogical.planSourceExpand(
        ...
        EveryRelationship(AnyOf(RelType("KNOWS"))),
        IRField("m")(CTNode),
        logicalNodeScan("n"),
        logicalNodeScan("m")
      )
  ...
  test("flat plan for init var expand") {
    ...
    val logicalPlan = mkLogical.planBoundedVarLengthExpand('n -> CTNode,
    ...
    val flatOp = mkFlat.boundedVarExpand(edgeScan.edge,
                                         edgeList,
                                         target,
                                         1,
                                         1,
  ...
  test("Construct selection") {
    val result = flatPlanner.process(
      mkLogical.planSelect(
        ...
        prev = mkLogical.projectField(IRField("foo")(CTString),
    ...
    result should equal(
      mkFlat.select(
        ...
        Set.empty,
        mkFlat.project(
          ProjectedField(Var("foo")(CTString),
                         Property(Var("n")(CTNode), PropertyKey("name"))(CTString)),
  ...
  test("Construct selection with several fields") {
    val result = flatPlanner.process(
      mkLogical.planSelect(
        ...
        prev = mkLogical.projectField(
          ...
          mkLogical.projectField(IRField("foo")(CTString),
                                 Property(Var("n")(CTNode), PropertyKey("name"))(CTString),
                                 logicalNodeScan("n", "Person"))
        )
      )
    ...
    val orderedContents =
      result.header.slots.map(_.content).collect { case content: FieldSlotContent => content.key }
    ...
    result should equal(
      mkFlat.select(
        ...
        Set.empty,
        mkFlat.project(
          ProjectedField(Var("baz")(CTInteger.nullable),
                         Property(Var("n")(CTNode), PropertyKey("age"))(CTInteger.nullable)),
          mkFlat.project(ProjectedField(Var("foo")(CTString),
                                        Property(Var("n")(CTNode), PropertyKey("name"))(CTString)),
                         flatNodeScan(Var("n")(CTNode), "Person"))
        )
      )
    )
    orderedContents should equal(
      IndexedSeq(Var("foo")(CTString), Var("n")(CTNode), Var("baz")(CTInteger)))
  }

  private def logicalNodeScan(nodeField: String, labelNames: String*) = {
    val labels = labelNames.map(Label)

    mkLogical.planNodeScan(IRField(nodeField)(CTNode),
                           EveryNode(AllOf(labels: _*)),
                           logicalStartOperator)
  }
