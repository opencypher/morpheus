...
class CAPSUnionGraphTest extends CAPSTestSuite {
    ...
    val nodeScan = NodeScan
      ...
      .from(
        CAPSRecords.create(
          ...
          Seq((0L, true, "Mats", 23L), (1L, false, "Martin", 42L), (2L, false, "Max", 1337L), (3L, false, "Stefan", 9L))
        ))
  ...
  private def initPersonReadsBookGraph: CAPSGraph = {
    CAPSUnionGraph(
      ...
      CAPSUnionGraph(TestGraph(`:Book`).graph, CAPSUnionGraph(TestGraph(`:Person`).graph)))
  }
}
