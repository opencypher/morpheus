...
class CAPSPatternGraphTest extends CAPSTestSuite {
  ...
  test("project pattern graph") {
    ...
    val person = inputGraph.cypher("""MATCH (a:Swedish)
  ...
  test("project pattern graph with relationship") {
    ...
    val person = inputGraph.cypher("""MATCH (a:Person:Swedish)-[r]->(b)
  ...
  test("project pattern graph with created node with labels") {
    ...
    graph
      .cypher("MATCH (n:Swede) RETURN labels(n)")
      .records
      .toCypherMaps
      .collect()
      .toSet should equal(
  ...
  test("Supports .cypher node scans") {
    ...
    patternGraph
      .cypher("MATCH (p:Person {name: 'Mats'}) RETURN p.luckyNumber")
      .records
      .toMaps should equal(Bag(CypherMap("p.luckyNumber" -> 23)))
  }
  ...
  test("Supports node scans from ad-hoc table") {
    ...
    patternGraph.nodes("n", CTNode("Person")).compact.toMaps should equal(
      Bag(
        CypherMap("n" -> 0L),
        CypherMap("n" -> 1L),
        CypherMap("n" -> 10L)
  ...
  ignore("Supports node scans from ad-hoc table 2") {
    ...
    val fields = Seq(
      ...
      ProjectedField('cName -> CTString.nullable,
  ...
  test("Supports node scans when different variables have the same property keys") {
    ...
    patternGraph.nodes("n", CTNode).toMaps should equal(
      Bag(
        ...
        CypherMap("n"          -> 1L,
                  "n.name"     -> "EmployeePeter",
                  "n:Person"   -> false,
                  "n:Employee" -> true),
        CypherMap("n"          -> 10L,
                  "n.name"     -> "PersonSusanna",
                  "n:Person"   -> true,
                  "n:Employee" -> false),
        CypherMap("n"          -> 11L,
                  "n.name"     -> "EmployeeSusanna",
                  "n:Person"   -> false,
                  "n:Employee" -> true)
      ))
  }
