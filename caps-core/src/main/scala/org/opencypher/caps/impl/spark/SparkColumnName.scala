...
object SparkColumn {
  def from(df: DataFrame): RecordSlot => Column =
    (record) => df.col(SparkColumnName.of(record.content))
}
...
object SparkColumnName {
  ...
  def of(slot: SlotContent): String = {
    val builder = slot match {
      ...
      case ProjectedExpr(p: Property) =>
        new NameBuilder() += None += p.withoutType + p.cypherType.material.name
