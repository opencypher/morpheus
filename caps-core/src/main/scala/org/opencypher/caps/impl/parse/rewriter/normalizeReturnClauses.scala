...
case class normalizeReturnClauses(mkException: (String, InputPosition) => CypherException)
    extends Rewriter {
    ...
    case clause @ Return(distinct, ri @ ReturnItems(_, items), gri, None, skip, limit, _) =>
      val (aliasProjection, finalProjection) = items.map {
        ...
        case item =>
          ...
          val newVariable =
            Variable(FreshIdNameGenerator.name(item.expression.position))(item.expression.position)
