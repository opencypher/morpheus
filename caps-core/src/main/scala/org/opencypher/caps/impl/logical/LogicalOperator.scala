...
final case class LogicalPatternGraph(name: String, schema: Schema, pattern: GraphOfPattern)
...
case class ConstructedRelationship(v: Var, source: Var, target: Var, typ: String)
...
final case class ExpandSource(
  ...
  override def clone(newLhs: LogicalOperator = lhs,
                     newRhs: LogicalOperator = rhs): LogicalOperator =
...
final case class ValueJoin(lhs: LogicalOperator,
                           rhs: LogicalOperator,
...
final case class ExpandInto(source: Var,
                            ...
                            types: EveryRelationship,
                            target: Var,
                            sourceOp: LogicalOperator)(override val solved: SolvedQueryModel[Expr])
    extends ExpandOperator {
...
final case class Aggregate(aggregations: Set[(Var, Aggregator)],
                           group: Set[Var],
                           in: LogicalOperator)(override val solved: SolvedQueryModel[Expr])
    extends StackingLogicalOperator {
  ...
  override def pretty(depth: Int): String =
    s"""${prefix(depth)} Aggregate(aggregations = ${aggregations
         .map(p => s"${p._2} AS ${p._1}")
         .mkString(", ")})
       #${in.pretty(depth + 1)}""".stripMargin('#')
...
final case class Start(sourceGraph: LogicalGraph, fields: Set[Var])(
  ...
  override def pretty(depth: Int): String =
    s"${prefix(depth)} Start(at = $sourceGraph, with = $fields)"
