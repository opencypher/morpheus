...
final case class SchemaTyper(schema: Schema) {
  ...
  def inferOrThrow(expr: Expression,
                   tracker: TypeTracker = TypeTracker.empty): TyperResult[CypherType] =
...
object SchemaTyper {
  ...
  def process[R: _hasSchema: _keepsErrors: _hasTracker: _logsTypes](
      expr: Expression): Eff[R, CypherType] = expr match {
    ...
    case RetypingPredicate(left, rhs) =>
      for {
        result <- if (left.isEmpty) pure[R, CypherType](CTBoolean)
        else processAndsOrs(expr, left.toVector)
  ...
  private def numericTypeOrError(expr: Expression with BinaryOperatorExpression,
                                 lhsType: CypherType,
  ...
  private implicit class LiftedMonoid[R, A](val monoid: Monoid[A])
      extends AnyVal
      with Monoid[Eff[R, A]] {
      ...
      for {
        ...
        computedType <- signatures.map(_._2).reduceLeftOption(_ join _) match {
          case Some(outputType) =>
            pure[R, CypherType](
              if (argTypes.exists(_.isNullable)) outputType.nullable else outputType)
    ...
    protected def selectSignaturesFor[R: _hasSchema: _keepsErrors: _hasTracker: _logsTypes](
      ...
      for {
        ...
        eligible <- pure(signatures.flatMap { sig =>
          ...
          val compatibleTypes =
            sigArgTypes.forall(((_: CypherType) couldBeSameTypeAs (_: CypherType)).tupled)
  ...
  private case object FunctionInvocationTyper
      extends SignatureBasedInvocationTyper[FunctionInvocation] {
    override protected def generateSignaturesFor[
        R: _hasSchema: _keepsErrors: _hasTracker: _logsTypes](
