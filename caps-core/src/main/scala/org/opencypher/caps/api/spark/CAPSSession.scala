...
import org.opencypher.caps.impl.spark.physical.{
  CAPSResultBuilder,
  PhysicalPlanner,
...
sealed class CAPSSession private (val sparkSession: SparkSession,
  ...
  def storeGraphAt(graph: CAPSGraph,
                   pathOrUri: String,
                   mode: PersistMode = CreateOrFail): CAPSGraph =
  ...
  override def cypher(graph: Graph,
                      query: String,
                      queryParameters: Map[String, CypherValue]): Result = {
    ...
    val ir = IRBuilder(stmt)(
      IRBuilderContext.initial(query, allParameters, semState, ambientGraph, sourceAt))
    logStageProgress("Done!")

    logStageProgress("Logical plan ...", false)
    val logicalPlannerContext =
      LogicalPlannerContext(graph.schema, Set.empty, ir.model.graphs.andThen(sourceAt))
  ...
  private def planStart(graph: Graph, fields: Set[Var]): LogicalOperator = {
    ...
    producer.planStart(LogicalExternalGraph(ambientGraph.name, ambientGraph.uri, graph.schema),
                       fields)
  }

  def filter(graph: Graph,
             in: Records,
             expr: Expr,
             queryParameters: Map[String, CypherValue]): Records = {
