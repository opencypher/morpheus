...
trait RegisterSyntax {
  def key[D, K](
      defn: D)(implicit register: Register[_] { type Def = D; type Key = K }): register.Key =
    register.key(defn)

  implicit def registerSyntax[C, R, K, D](coll: C)(implicit
                                                   register: Register[C] {
                                                     type Ref = R; type Key = K; type Def = D
                                                   }): RegisterOps[C, R, K, D] =
    new RegisterOps[C, R, K, D](coll)
}

final class RegisterOps[C, R, K, D](coll: C)(implicit
