...
final case class ResultBlock[E](
  ...
  def select(fields: Set[IRField]): ResultBlock[E] =
    copy(binds = binds.select(fields),
         nodes = nodes intersect fields,
         relationships = relationships intersect fields)
}
...
final case class OrderedFieldsAndGraphs[E](fieldsOrder: IndexedSeq[IRField] = IndexedSeq.empty,
...
case object FieldsInOrder {
  def apply[E](fields: IRField*): OrderedFieldsAndGraphs[E] =
    OrderedFieldsAndGraphs[E](fields.toIndexedSeq)
