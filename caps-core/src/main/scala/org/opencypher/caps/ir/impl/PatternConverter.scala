...
final class PatternConverter(val parameters: Map[String, CypherValue]) extends AnyVal {
  ...
  private def convertElement(p: ast.PatternElement, knownTypes: Map[Expression, CypherType]): Result[IRField] =
    p match {
      ...
      case ast.RelationshipChain(
        ...
        for {
          ...
          _ <- modify[Pattern[Expr]] { given =>
            ...
            val upper =
              range.upper.map(_.value.intValue()).getOrElse(Raise.notYetImplemented("unbounded variable length"))
