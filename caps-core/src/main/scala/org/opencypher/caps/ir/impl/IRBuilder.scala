...
object IRBuilder extends CompilationStage[ast.Statement, CypherQuery[Expr], IRBuilderContext] {
  ...
  private def buildIR[R: _mayFail: _hasContext](
      s: ast.Statement): Eff[R, Option[CypherQuery[Expr]]] =
    s match {
  ...
  def registerIRGraph[R: _hasContext](c: Clause): Eff[R, IRGraph] = {
    for {
      ...
      graph <- {
        val currentGraph = context.semanticState.recordedContextGraphs
          .find {
            case (clause, _) =>
  ...
  private def convertClause[R: _mayFail: _hasContext](c: ast.Clause): Eff[R, Vector[BlockRef]] = {
    ...
    c match {
      ...
      case ast.With(_,
                    ...
                    GraphReturnItems(_, gItems),
                    orderBy,
                    skip,
                    limit,
                    where) if !items.exists(_.expression.containsAggregate) =>
        for {
          irGraph    <- registerIRGraph(c)
          ...
          refs <- {
            val (projectRef, projectReg) =
              registerProjectBlock(context, fieldExprs, graphs, given, irGraph)
            val appendList = (list: Vector[BlockRef]) =>
              pure[R, Vector[BlockRef]](projectRef +: list)
      ...
      case ast.With(_, ast.ReturnItems(_, items), GraphReturnItems(_, Seq()), _, _, _, None) =>
        for {
          ...
          refs <- {
            ...
            val aggBlock = AggregationBlock[Expr](after,
                                                  Aggregations(agg.toSet),
                                                  group.map(_._1).toSet,
                                                  context.ambientGraph)
            val (ref2, reg2) = reg1.register(aggBlock)

            put[R, IRBuilderContext](context.copy(blocks = reg2)) >> pure[R, Vector[BlockRef]](
              Vector(ref1, ref2))
          }
        } yield refs
      ...
      case ast.Return(distinct, ast.ReturnItems(_, items), graphItems, orderBy, skip, limit, _) =>
        for {
          ...
          context    <- get[R, IRBuilderContext]
          refs <- {
            val (projectRef, projectReg) = registerProjectBlock(context,
                                                                fieldExprs,
                                                                distinct = distinct,
                                                                source = irGraph,
                                                                graphs = graphs)
          ...
          refs2 <- {
            ...
            val result = ResultBlock[Expr](Set(refs.last),
                                           orderedFields,
                                           Set.empty,
                                           Set.empty,
                                           context.ambientGraph)
            val (resultRef, resultReg) = context2.blocks.register(result)
            put[R, IRBuilderContext](context.copy(blocks = resultReg)) >> pure[R, Vector[BlockRef]](
              refs :+ resultRef)
          }
        } yield refs2
      ...
      case ast.Return(distinct, ast.DiscardCardinality(), graphItems, _, _, _, _) =>
        for {
          ...
          context <- get[R, IRBuilderContext]
          refs <- {
            val (ref, reg) = registerProjectBlock(context,
                                                  Vector.empty,
                                                  distinct = distinct,
                                                  source = irGraph,
                                                  graphs = graphs)
            val orderedFields =
              OrderedFieldsAndGraphs[Expr](Vector.empty, graphs.map(_.toNamedGraph).toSet)
            val returns =
              ResultBlock[Expr](Set(ref), orderedFields, Set.empty, Set.empty, context.ambientGraph)
  ...
  private def convertGraphReturnItems[R: _hasContext](
      maybeItems: Option[GraphReturnItems]): Eff[R, Vector[IRGraph]] = {
  ...
  private def registerProjectBlock(context: IRBuilderContext,
    ...
    for {
      ...
      refs <- {
        if (sortItems.isEmpty && skipExpr.isEmpty && limitExpr.isEmpty)
          pure[R, Vector[BlockRef]](Vector())
        else {
          val blockRegistry = context.blocks
          val after         = blockRegistry.lastAdded.toSet

          val orderAndSliceBlock =
            OrderAndSliceBlock[Expr](after, sortItems, skipExpr, limitExpr, context.ambientGraph)
          val (ref, reg) = blockRegistry.register(orderAndSliceBlock)
          put[R, IRBuilderContext](context.copy(blocks = reg)) >> pure[R, Vector[BlockRef]](
            Vector(ref))
        }
      }
    } yield refs
  ...
  private def convertGraphReturnItem[R: _hasContext](item: ast.GraphReturnItem): Eff[R, IRGraph] =
    item match {
      case ast.NewContextGraphs(source: GraphAtAs, target)
          if target.isEmpty || target.contains(source) =>
          ...
          result <- graph match {
            ...
            case ast.GraphAtAs(url, _, _) =>
              ...
              put[R, IRBuilderContext](newContext) >>
                pure[R, IRGraph](
                  IRExternalGraph(graphName, newContext.schemaFor(graphName), graphURI))
  ...
  private def convertReturnItem[R: _mayFail: _hasContext](
      item: ast.ReturnItem): Eff[R, (IRField, Expr)] = item match {
    ...
    case _ =>
      Raise.invalidArgument(s"${AliasedReturnItem.getClass} or ${UnaliasedReturnItem.getClass}",
                            s"${item.getClass}")
  }
  ...
  private def convertPattern[R: _hasContext](p: ast.Pattern): Eff[R, Pattern[Expr]] = {
    for {
      ...
      result <- {
        ...
        put[R, IRBuilderContext](context.copy(knownTypes = patternTypes)) >> pure[R, Pattern[Expr]](
          pattern)
      }
    } yield result
  }
  ...
  private def convertExpr[R: _mayFail: _hasContext](
      e: Option[ast.Expression]): Eff[R, Option[Expr]] =
    for {
      context <- get[R, IRBuilderContext]
    } yield
      e match {
  ...
  private def convertWhere[R: _mayFail: _hasContext](
      where: Option[ast.Where]): Eff[R, AllGiven[Expr]] = where match {
  ...
  private def convertSortItem[R: _mayFail: _hasContext](
      item: ast.SortItem): Eff[R, SortItem[Expr]] = {
    item match {
