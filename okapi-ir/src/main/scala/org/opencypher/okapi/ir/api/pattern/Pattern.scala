...
case object Pattern {
  ...
  def node[E](node: IRField): Pattern =
    Pattern(fields = Set(node), topology = ListMap.empty)
}
...
final case class Pattern(
  ...
  def ++(other: Pattern): Pattern = {
    ...
    val conflicts = topology.keySet
      .intersect(other.topology.keySet)
      .filter(k => topology(k) != other.topology(k))
  ...
  private def verifyFieldTypes(
    map1: Map[String, CypherType],
    map2: Map[String, CypherType]
  ): Unit = {
    (map1.keySet ++ map2.keySet).foreach { f =>
      map1.get(f) -> map2.get(f) match {
        case (Some(t1), Some(t2)) =>
          if (t1 != t2)
            throw PatternConversionException(
              s"Expected disjoint patterns but found conflicting elements $f"
            )
        case _ =>
      }
    }
  ...
  def connectionsFor(node: IRField): Map[IRField, Connection] = {
    topology.filter { case (_, c) =>
      c.endpoints.contains(node)
    }
  }
  ...
  def withConnection(
    ...
    propertiesOpt: Option[MapExpression] = None
  ): Pattern = {
    ...
    if (topology.get(key).contains(connection)) withProperties
    else withProperties.copy(topology = topology.updated(key, connection))
  }
  ...
  def withElement(
    ...
    if (fields(field)) withProperties
    else withProperties.copy(fields = fields + field)
  }

  def withBaseField(field: IRField, baseOpt: Option[IRField]): Pattern =
    baseOpt match {
      case Some(base) if fields.contains(field) =>
        copy(baseFields = baseFields.updated(field, base))
      case _ => this
    }

  def components: Set[Pattern] = {
    val _fields = fields.foldLeft(Map.empty[IRField, Int]) { case (m, f) =>
      m.updated(f, m.size)
    }
    val components = nodes.foldLeft(Map.empty[Int, Pattern]) { case (m, f) =>
      m.updated(_fields(f), Pattern.node(f))
    }
  ...
  private def computeComponents(
    ...
    case Seq((field, connection), tail @ _*) =>
      ...
      if (links.isEmpty) {
        ...
        val newFields = endpoints.foldLeft(fieldToComponentIndex) { case (m, endpoint) =>
          m.updated(endpoint, count)
        }
        ...
        val newFields =
          fieldToComponentIndex.mapValues(l => if (links(l)) newCount else l)
        computeComponents(tail, newComponents, newCount, newFields)
      }
