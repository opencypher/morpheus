...
/**
  ...
  *
  * @see
  *   [[http://neo4j.com/docs/developer-manual/current/cypher/syntax/expressions/ Cypher Expressions in the Neo4j Manual]]
  */
sealed abstract class Expr extends AbstractTreeNode[Expr] {
  ...
  /**
    * Returns the node/relationship that this expression is owned by, if it is owned. A
    * node/relationship owns its label/key/property mappings
    */
  ...
  /**
    * When `nullInNullOut` is true, then the expression evaluates to `null`, if any of its inputs
    * evaluate to `null`.
    *
...
object Var {
  ...
  def apply(name: String)(cypherType: CypherType = CTAny): Var =
    cypherType match {
    ...
    maybeType match {
      case Some(typ) =>
        propagationType match {
          ...
          case Some(AnyNullable) =>
            if (exprs.exists(_.cypherType.isNullable)) typ.nullable else typ
          case Some(AllNullable) =>
            if (exprs.forall(_.cypherType.isNullable)) typ.nullable else typ
          case None => typ
        }
      case None =>
        if (children.exists(_.cypherType == CTNull)) CTNull
        else
          throw NoSuitableSignatureForExpr(
            s"Type signature ${getClass.getSimpleName}($materialTypes) is not supported."
          )
    }
  }
...
final case class ListSegment(index: Int, listVar: Var) extends Var with TypeValidatedExpr {
  ...
  override def signature(
    inputCypherTypes: Seq[CypherType]
  ): Option[CypherType] = inputCypherTypes.head match {
...
final case class RelationshipVar(name: String)(
  val cypherType: CypherType = CTRelationship
) extends ReturnItem {
  override def owner: Option[Var] = Some(this)
  ...
  override def withOwner(expr: Var): RelationshipVar = expr match {
    ...
    case other =>
      other.cypherType match {
        case r if r.subTypeOf(CTRelationship.nullable) =>
          RelationshipVar(other.name)(r)
...
final case class SimpleVar(name: String)(val cypherType: CypherType) extends ReturnItem {
  ...
  override def withOwner(expr: Var): SimpleVar =
    SimpleVar(expr.name)(expr.cypherType)
}
...
object FlattenOps {
  ...
  implicit class RichExpressions(exprs: Traversable[Expr]) {
    ...
    def flattenExprs[E <: Expr: ClassTag]: List[Expr] = {
      @tailrec def flattenRec(
        es: List[Expr],
        result: Set[Expr] = Set.empty
      ): Set[Expr] = {
        es match {
          case Nil => result
          case h :: tail =>
...
final case class Ands(_exprs: List[Expr]) extends Expr {
  ...
  override def withoutType =
    s"ANDS(${_exprs.map(_.withoutType).mkString(", ")})"
...
sealed trait PredicateExpression extends TypeValidatedExpr {
  ...
  override def signature(
    inputCypherTypes: Seq[CypherType]
  ): Option[CypherType] = Some(CTBoolean)
}
...
final case class HasLabel(node: Expr, label: Label) extends PredicateExpression {
  ...
  override def signature(
    ...
    case u: CTUnion if u.alternatives.forall(_.subTypeOf(CTNode)) =>
      Some(CTBoolean)
    case _ => None
  }
}
...
final case class HasType(rel: Expr, relType: RelType) extends PredicateExpression {
  ...
  override def signature(
    ...
    case u: CTUnion if u.alternatives.forall(_.subTypeOf(CTRelationship)) =>
      Some(CTBoolean)
    case _ => None
  }
}
...
final case class StartsWith(lhs: Expr, rhs: Expr)
    extends BinaryPredicate
    with BinaryStrExprSignature {
  override def op: String = "STARTS WITH"
}

final case class EndsWith(lhs: Expr, rhs: Expr)
    extends BinaryPredicate
    with BinaryStrExprSignature {
  override def op: String = "ENDS WITH"
}

final case class Contains(lhs: Expr, rhs: Expr)
    extends BinaryPredicate
    with BinaryStrExprSignature {
...
sealed trait InequalityExprSignature {
  def signature(lhsType: CypherType, rhsType: CypherType): Option[CypherType] =
    lhsType -> rhsType match {
      case (n1, n2) if n1.subTypeOf(CTNumber) && n2.subTypeOf(CTNumber) =>
        Some(CTBoolean)
...
sealed trait BinaryExpr extends TypeValidatedExpr {
  ...
  override final def withoutType: String =
    s"${lhs.withoutType} $op ${rhs.withoutType}"
  def lhs: Expr
  def rhs: Expr
  def op: String
  ...
  override def signature(
    inputCypherTypes: Seq[CypherType]
  ): Option[CypherType] = signature(inputCypherTypes.head, inputCypherTypes(1))
}

sealed trait BinaryPredicate extends BinaryExpr {
  override def propagationType: Option[PropagationType] = Some(
    NullOrAnyNullable
  )
}
...
final case class RegexMatch(lhs: Expr, rhs: Expr)
    extends BinaryPredicate
    with BinaryStrExprSignature {
  override def op: String = "=~"
}

final case class LessThan(lhs: Expr, rhs: Expr)
    extends BinaryPredicate
    with InequalityExprSignature {
  override val op = "<"
}

final case class LessThanOrEqual(lhs: Expr, rhs: Expr)
    extends BinaryPredicate
    with InequalityExprSignature {
  override val op = "<="
}

final case class GreaterThan(lhs: Expr, rhs: Expr)
    extends BinaryPredicate
    with InequalityExprSignature {
  override val op = ">"
}

final case class GreaterThanOrEqual(lhs: Expr, rhs: Expr)
    extends BinaryPredicate
    with InequalityExprSignature {
  override val op = ">="
}
...
final case class In(lhs: Expr, rhs: Expr) extends BinaryPredicate {
  ...
  override def computeCypherType: CypherType =
  ...
  def signature(lhsType: CypherType, rhsType: CypherType): Option[CypherType] =
    None // signatures not used as In to special
}
...
final case class ElementProperty(propertyOwner: Expr, key: PropertyKey)(
  val cypherType: CypherType
) extends Property {

  override def withOwner(v: Var): ElementProperty =
    ElementProperty(v, key)(cypherType)
  ...
  val cypherType: CypherType = propertyOwner.cypherType match {
    ...
    case other =>
      throw IllegalArgumentException(
        s"Map property needs to be defined on a map. `$propertyOwner` is of type `$other`."
      )
  }
...
final case class LocalDateTimeProperty(propertyOwner: Expr, key: PropertyKey) extends Property {
  ...
  override def withOwner(v: Var): LocalDateTimeProperty =
    LocalDateTimeProperty(v, key)
...
final case class MapExpression(items: Map[String, Expr]) extends Expr {
  ...
  override def cypherType: CypherType = CTMap(items.map { case (key, value) =>
    key -> value.cypherType
  })
}
...
final case class MapProjection(
  ...
  def withoutType: String =
    s"{${items.map(x => x._1 + ":" + x._2.withoutType)}}"
}
...
final case class Add(lhs: Expr, rhs: Expr) extends ArithmeticExpr {
  ...
  override def signature(
    lhsType: CypherType,
    rhsType: CypherType
  ): Option[CypherType] = lhsType -> rhsType match {
    ...
    case (left, right) =>
      BigDecimalSignatures.arithmeticSignature(Addition)(Seq(left, right))
  }

  def listConcatJoin(lhsType: CTList, rhsType: CypherType): CypherType =
    lhsType -> rhsType match {
...
final case class Multiply(lhs: Expr, rhs: Expr) extends ArithmeticExpr {
  ...
  override def signature(
    ...
    case (left, right) =>
      BigDecimalSignatures.arithmeticSignature(Multiplication)(Seq(left, right))
  }
}
...
final case class Divide(lhs: Expr, rhs: Expr) extends ArithmeticExpr {
  ...
  override def signature(
    ...
    case (CTDuration, CTInteger)   => Some(CTDuration)
    case (left, right) =>
      BigDecimalSignatures.arithmeticSignature(Division)(Seq(left, right))
  }
}
...
sealed trait FunctionExpr extends TypeValidatedExpr {
  ...
  override final def withoutType =
    s"$name(${exprs.map(_.withoutType).mkString(", ")})"
...
sealed trait NullaryFunctionExpr extends FunctionExpr {
  ...
  override def signature(
    inputCypherTypes: Seq[CypherType]
  ): Option[CypherType] = Some(cypherType)
}
...
sealed trait UnaryFunctionExpr extends FunctionExpr {
  ...
  def signature(inputCypherTypes: Seq[CypherType]): Option[CypherType] =
    signature(inputCypherTypes.head)
}

final case class Head(expr: Expr) extends UnaryFunctionExpr {
  override def signature(inputCypherType: CypherType): Option[CypherType] =
    inputCypherType match {
      case CTList(inner) => Some(inner)
      case _             => None
    }
}
...
final case class Exists(expr: Expr) extends UnaryFunctionExpr {
  def signature(inputCypherType: CypherType): Option[CypherType] = Some(
    CTBoolean
  )
}
...
final case class Keys(expr: Expr) extends UnaryFunctionExpr {
  ...
  def signature(inputCypherType: CypherType): Option[CypherType] =
    inputCypherType match {
      case CTNode(_, _) | CTRelationship(_, _) | CTMap(_) =>
        Some(CTList(CTString))
      case _ => None
    }
}
...
final case class ToString(expr: Expr) extends UnaryFunctionExpr {
  ...
  def signature(inputCypherType: CypherType): Option[CypherType] =
    inputCypherType match {
      ...
      case x if x.subTypeOf(CTUnion(CTNumber, CTTemporal, CTBoolean)) =>
        Some(CTString)
      case _ => None
    }
}
...
final case class BigDecimal(expr: Expr, precision: Long, scale: Long) extends UnaryFunctionExpr {
  if (scale > precision) {
    throw IllegalArgumentException(
      "Greater precision than scale",
      s"precision: $precision, scale: $scale"
    )
  }
  override def propagationType: Option[PropagationType] = Some(AnyNullable)
  ...
  def signature(inputCypherType: CypherType): Option[CypherType] =
    inputCypherType match {
      case x if x.subTypeOf(CTNumber) =>
        Some(CTBigDecimal(precision.toInt, scale.toInt))
      case _ => None
    }
}
...
final case class Coalesce(exprs: List[Expr]) extends FunctionExpr {
  ...
  override def signature(
    inputCypherTypes: Seq[CypherType]
  ): Option[CypherType] = Some(inputCypherTypes.reduceLeft(_ | _))
    ...
    case CTUnion(options) =>
      options
        .map {
          ...
          case _ =>
            throw IllegalArgumentException(
              "input cypher type to resolve to a list",
              expr.cypherType
            )
        }
        .reduceLeft(_ | _)
...
final case class Properties(expr: Expr)(override val cypherType: CypherType)
    extends UnaryFunctionExpr {
...
final case class Range(from: Expr, to: Expr, o: Option[Expr]) extends FunctionExpr {
  ...
  override def signature(
    inputCypherTypes: Seq[CypherType]
  ): Option[CypherType] = inputCypherTypes match {
...
final case class ShiftLeft(value: Expr, shiftBits: IntegerLit) extends BinaryExpr {
  ...
  override def signature(
    lhsType: CypherType,
    rhsType: CypherType
  ): Option[CypherType] = Some(value.cypherType)
}
...
final case class BitwiseAnd(lhs: Expr, rhs: Expr) extends BinaryExpr {
  ...
  override def signature(
    lhsType: CypherType,
    rhsType: CypherType
  ): Option[CypherType] = Some(lhs.cypherType | rhs.cypherType)
}
...
final case class Atan2(expr1: Expr, expr2: Expr) extends FunctionExpr {
  ...
  override def signature(
    ...
    case Seq(c1, c2) if c1.subTypeOf(CTNumber) && c2.subTypeOf(CTNumber) =>
      Some(CTFloat)
    case _ => None
  }
}
...
final case class PercentileCont(expr: Expr, percentile: Expr) extends Aggregator {
  ...
  override def withoutType: String =
    s"percentileCont(${expr.withoutType}, ${percentile.withoutType})"
...
final case class PercentileDisc(expr: Expr, percentile: Expr) extends Aggregator {
  ...
  override def withoutType: String =
    s"percentileDisc(${expr.withoutType}, ${percentile.withoutType})"
...
final case class ListLit(v: List[Expr]) extends Lit[List[Expr]] {
  override def cypherType: CypherType = CTList(
    v.foldLeft(CTVoid: CypherType)(_ | _.cypherType)
  )
}

sealed abstract class ListSlice(maybeFrom: Option[Expr], maybeTo: Option[Expr])
    extends TypeValidatedExpr {
  def list: Expr
  override def withoutType: String =
    s"${list.withoutType}[${maybeFrom.map(_.withoutType).getOrElse("")}..${maybeTo.map(_.withoutType).getOrElse("")}]"

  override def propagationType: Option[PropagationType] = Some(
    NullOrAnyNullable
  )
  def signature(inputCypherTypes: Seq[CypherType]): Option[CypherType] =
    inputCypherTypes.head match {
      case CTList(_) if inputCypherTypes.tail.forall(_.couldBeSameTypeAs(CTInteger)) =>
        Some(list.cypherType)
      case _ => None
    }
}

final case class ListSliceFromTo(list: Expr, from: Expr, to: Expr)
    extends ListSlice(Some(from), Some(to)) {
...
final case class ListComprehension(
  variable: Expr,
  innerPredicate: Option[Expr],
  extractExpression: Option[Expr],
  expr: Expr
) extends Expr {
  override def withoutType: String = {
...
final case class ListReduction(
  accumulator: Expr,
  variable: Expr,
  reduceExpr: Expr,
  initExpr: Expr,
  ...
  def exprs: List[Expr] =
    List(initExpr, reduceExpr, list) // only exprs which need to be type-checked
  def signature(inputCypherTypes: Seq[CypherType]): Option[CypherType] =
    inputCypherTypes match {
      case Seq(initType: CypherType, reduceStepType: CypherType, CTList(_))
          if initType.couldBeSameTypeAs(reduceStepType) =>
        Some(initType)
      case _ => None
    }
}

sealed abstract class IterablePredicateExpr(
  variable: LambdaVar,
  predicate: Expr,
  list: Expr
) extends TypeValidatedExpr {
...
final case class ListFilter(variable: LambdaVar, predicate: Expr, list: Expr)
    extends TypeValidatedExpr {
  override def withoutType: String =
    s"filter($variable IN $list WHERE $predicate)"
  def exprs: List[Expr] = List(variable, predicate, list)
...
final case class ListAny(variable: LambdaVar, predicate: Expr, list: Expr)
    extends IterablePredicateExpr(variable, predicate, list) {
  override def withoutType: String = s"any(${super.withoutType})"
}

final case class ListNone(variable: LambdaVar, predicate: Expr, list: Expr)
    extends IterablePredicateExpr(variable, predicate, list) {
  override def withoutType: String = s"none(${super.withoutType})"
}

final case class ListAll(variable: LambdaVar, predicate: Expr, list: Expr)
    extends IterablePredicateExpr(variable, predicate, list) {
  override def withoutType: String = s"all(${super.withoutType})"
}

final case class ListSingle(variable: LambdaVar, predicate: Expr, list: Expr)
    extends IterablePredicateExpr(variable, predicate, list) {
  override def withoutType: String = s"single(${super.withoutType})"
}

final case class ContainerIndex(container: Expr, index: Expr)(
  val cypherType: CypherType
) extends Expr {

  override def withoutType: String =
    s"${container.withoutType}[${index.withoutType}]"
...
sealed abstract class TemporalInstant(
  expr: Option[Expr],
  outputCypherType: CypherType
  ...
  override def signature(
    inputCypherTypes: Seq[CypherType]
  ): Option[CypherType] = inputCypherTypes.headOption match {
...
final case class LocalDateTime(maybeExpr: Option[Expr])
    extends TemporalInstant(maybeExpr, CTLocalDateTime)
...
final case class Duration(expr: Expr) extends UnaryFunctionExpr {
  ...
  override def signature(cypherType: CypherType): Option[CypherType] =
    cypherType match {
...
final case class ExistsPatternExpr(targetField: Var, ir: CypherQuery) extends Expr {
...
final case class CaseExpr(
  alternatives: List[(Expr, Expr)],
  default: Option[Expr]
)(val cypherType: CypherType)
    extends Expr {

  override val children: Array[Expr] = (default ++ alternatives.flatMap { case (cond, value) =>
    Seq(cond, value)
  }).toArray
