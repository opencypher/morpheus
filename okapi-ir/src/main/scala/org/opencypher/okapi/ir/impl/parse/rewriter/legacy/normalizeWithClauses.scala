...
/**
  * This rewriter normalizes the scoping structure of a query, ensuring it is able to be correctly
  * processed for semantic checking. It makes sure that all return items in a WITH clauses are
  * aliased.
  *
  * It also replaces expressions and subexpressions in ORDER BY and WHERE to use aliases introduced
  * by the WITH, where possible.
  *
  ...
  */
case class normalizeWithClauses(
  ...
  private val clauseRewriter: (Clause => Clause) = {
    ...
    case clause @ With(_, ri: ReturnItems, None, _, _, None) =>
      val (unaliasedReturnItems, aliasedReturnItems) = partitionReturnItems(
        ri.items
      )
    ...
    case clause @ With(
      ...
      val existingAliases =
        aliasedReturnItems.map(i => i.expression -> i.alias.get.copyId).toMap
      val updatedOrderBy = orderBy.map(aliasOrderBy(existingAliases, _))
      val updatedWhere = where.map(aliasWhere(existingAliases, _))

      clause.copy(
        returnItems = ri.copy(items = initialReturnItems)(ri.position),
  ...
  /**
    * Aliases return items if possible. Return a tuple of unaliased (because impossible) and aliased
    * (because they already were aliases or we just introduced an alias for them) return items.
    */
  private def partitionReturnItems(
    returnItems: Seq[ReturnItem]
  ): (Seq[ReturnItem], Seq[AliasedReturnItem]) =
    returnItems.foldLeft(
      (Vector.empty[ReturnItem], Vector.empty[AliasedReturnItem])
      ...
      item match {
        ...
        case i if i.alias.isDefined =>
          (
            unaliasedItems,
            aliasedItems :+ AliasedReturnItem(
              item.expression,
  ...
  private def aliasOrderBy(
    existingAliases: Map[Expression, LogicalVariable],
    originalOrderBy: OrderBy
  ): OrderBy = {
    val updatedSortItems = originalOrderBy.sortItems.map {
      aliasSortItem(existingAliases, _)
    }
  ...
  private def aliasSortItem(
    existingAliases: Map[Expression, LogicalVariable],
    sortItem: SortItem
  ): SortItem = {
    sortItem match {
      case AscSortItem(expression) =>
        AscSortItem(aliasExpression(existingAliases, expression))(
          sortItem.position
        )
      case DescSortItem(expression) =>
        DescSortItem(aliasExpression(existingAliases, expression))(
          sortItem.position
        )
    }
  }
  ...
  private def aliasWhere(
    existingAliases: Map[Expression, LogicalVariable],
    originalWhere: Where
  ): Where = {
    Where(aliasExpression(existingAliases, originalWhere.expression))(
      originalWhere.position
    )
  }

  /** Given a list of existing aliases, this rewrites expressions to use these where possible. */
  private def aliasExpression(
    existingAliases: Map[Expression, LogicalVariable],
    ...
    existingAliases.get(expression) match {
      ...
      case None =>
        val newExpression =
          expression.endoRewrite(topDown(Rewriter.lift { case subExpression: Expression =>
            existingAliases
              .get(subExpression)
              .map(_.copyId)
              .getOrElse(subExpression)
          }))
        newExpression
    }
  }
