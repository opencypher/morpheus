...
case object pushLabelsIntoScans extends Rewriter {
  ...
  private val rewriter = Rewriter.lift { case m: Match =>
    def containsHasLabel(e: Expression) = e.treeExists { case _: HasLabels =>
      true
    }

    val patternLabelMap =
      m.pattern.treeFold[Map[String, Set[LabelName]]](Map.empty) {
        case NodePattern(Some(Variable(name)), labels, _, _) =>
          acc => {
            ...
            val merge =
              (other: Map[String, Set[LabelName]]) => updated |+| other
            updated -> Some(merge)
          }
      }

    val whereLabelMap =
      m.where.treeFold[Map[String, Set[LabelName]]](Map.empty) {
        case Or(lhs, rhs) if containsHasLabel(lhs) || containsHasLabel(rhs) =>
          acc => acc -> None
        case Ors(children) if children.count(containsHasLabel) >= 1 =>
          acc => acc -> None
        case Not(child) if containsHasLabel(child) => acc => acc -> None
        case Xor(lhs, rhs) if containsHasLabel(lhs) || containsHasLabel(rhs) =>
          acc => acc -> None
    ...
    val where =
      m.where.endoRewrite(removeRedundantLabelFilters(labelMap)) match {
  ...
  private def addLabelsToNodePatterns(labelMap: Map[String, Set[LabelName]]) =
    bottomUp {
      case n @ NodePattern(Some(v), _, properties, base) if labelMap.contains(v.name) =>
        NodePattern(Some(v), labelMap(v.name).toSeq, properties, base)(
          n.position
        )
      case other => other
    }

  private def removeRedundantLabelFilters(
    labelMap: Map[String, Set[LabelName]]
  ) = bottomUp {
    case h @ HasLabels(Variable(name), labels)
        if labels.toSet subsetOf labelMap.getOrElse(name, Set.empty) =>
      True()(h.position)
