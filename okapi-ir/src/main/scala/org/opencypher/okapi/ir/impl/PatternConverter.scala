...
final class PatternConverter(irBuilderContext: IRBuilderContext) {
  ...
  def convertRelsPattern(
    ...
    convertElement(p.element, knownTypes, qualifiedGraphName)
      .runS(pattern)
      .value
  ...
  private def convertPattern(
    ...
    Foldable[List].sequence_[Result, Unit](
      p.patternParts.toList.map(convertPart(knownTypes, qualifiedGraphName))
    )

  @tailrec
  private def convertPart(
    knownTypes: Map[ast.Expression, CypherType],
    qualifiedGraphName: QualifiedGraphName
  )(p: ast.PatternPart): Result[Unit] = p match {
    case _: ast.AnonymousPatternPart =>
      stomp(convertElement(p.element, knownTypes, qualifiedGraphName))
    case ast.NamedPatternPart(_, part) =>
      convertPart(knownTypes, qualifiedGraphName)(part)
  }
  ...
  private def convertElement(
    ...
    p match {
      ...
      case np @ ast.NodePattern(
            vOpt,
            labels: Seq[ast.LabelName],
            propertiesOpt,
            baseNodeVar
          ) =>
        ...
        val baseNodeLabels =
          baseNodeCypherTypeOpt.map(_.toCTNode.labels).getOrElse(Set.empty)

        // labels defined in outside scope, passed in by IRBuilder
        val (knownLabels, qgnOption) = vOpt
          .flatMap(expr => knownTypes.get(expr))
          .flatMap {
        ...
        val nodeVar = vOpt match {
          ...
          case None =>
            FreshVariableNamer(np.position.offset, CTNode(allLabels, qgnOption))
        }
        ...
        for {
          ...
          _ <- modify[Pattern](
            _.withElement(element, extractProperties(propertiesOpt))
              .withBaseField(element, baseNodeField)
          )
        } yield element
      ...
      case rc @ ast.RelationshipChain(
        ...
        val relVar = createRelationshipVar(
          knownTypes,
          rc.position.offset,
          eOpt,
          types,
        ...
        for {
          ...
          rel <- pure(
            IRField(relVar.name)(
              if (rangeOpt.isDefined) CTList(relVar.cypherType)
              else relVar.cypherType
            )
          )
          _ <- modify[Pattern] { given =>
            ...
            rangeOpt match {
              case Some(Some(range)) =>
                ...
                val upper = range.upper
                  ...
                  .getOrElse(
                    throw NotImplementedException(
                      "Support for unbounded var-length not yet implemented"
                    )
                  )
                ...
                Endpoints.apply(source, target) match {
                  case _: IdenticalEndpoints =>
                    throw NotImplementedException(
                      "Support for cyclic var-length not yet implemented"
                    )
                  ...
                  case ends: DifferentEndpoints =>
                    dir match {
                      case OUTGOING =>
                        registered.withConnection(
                          rel,
                          DirectedRelationship(ends, OUTGOING),
                          convertedProperties
                        )
                      ...
                      case INCOMING =>
                        registered.withConnection(
                          rel,
                          DirectedRelationship(ends.flip, INCOMING),
                          convertedProperties
                        )
              ...
              case _ =>
                throw NotImplementedException(
                  s"Support for pattern conversion of $rc not yet implemented"
                )
            }
          }
        } yield target
      ...
      case x =>
        throw NotImplementedException(
          s"Support for pattern conversion of $x not yet implemented"
        )
    }
  ...
  private def createRelationshipVar(
    ...
    val baseRelTypes =
      baseRelCypherTypeOpt.map(_.toCTRelationship.types).getOrElse(Set.empty)

    // types defined in outside scope, passed in by IRBuilder
    val (knownRelTypes, qgnOption) = eOpt
      .flatMap(expr => knownTypes.get(expr))
      .flatMap {
    ...
    val rel = eOpt match {
      ...
      case None =>
        FreshVariableNamer(offset, CTRelationship(relTypes, qgnOption))
    }
    rel
  }
