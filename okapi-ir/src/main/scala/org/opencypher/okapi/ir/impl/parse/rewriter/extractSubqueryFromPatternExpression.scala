...
case class extractSubqueryFromPatternExpression(
  ...
  /**
    ...
    * to
    *
    * WHERE EXISTS { MATCH (a)-[e0]->(v0)-[e1]->(v1)... WHERE e0:R AND v0.foo = true RETURN a, true
    * } AND a.age > 20
    */
  private val instance = topDown(Rewriter.lift {
    case f @ FunctionInvocation(_, _, _, IndexedSeq(p: PatternExpression))
        if f.function == Exists =>
  ...
  private def rewritePatternExpression(p: PatternExpression): ExistsPattern = {
    ...
    val newPattern =
      Pattern(Seq(EveryPath(relationshipsPattern.element)))(patternPosition)

    val joinVariables =
      relationshipsPattern.element.treeFold(Seq.empty[LogicalVariable]) {
    ...
    val returnItems =
      joinVariables.map(v => AliasedReturnItem(v, v)(v.position))

    val trueVariable =
      Variable(UnNamedNameGenerator.name(p.position))(p.position)
    val returnItemsWithTrue = returnItems :+ AliasedReturnItem(
      True()(trueVariable.position),
      trueVariable
    ...
    ExistsPattern(
      Query(
        ...
        SingleQuery(
          Seq(
            Match(optional = false, newPattern, Seq.empty, None)(
              patternPosition
            )
              .endoRewrite(nameMatchPatternElements)
              .endoRewrite(
                normalizeMatchPredicates(getDegreeRewriting = false)
              ),
            Return(
              ReturnItems(includeExisting = false, returnItemsWithTrue)(
                patternPosition
              )
            )(patternPosition)
          )
...
case class ExistsPattern(query: Query, targetField: Variable)(
