...
object IRBuilder extends CompilationStage[ast.Statement, CypherStatement, IRBuilderContext] {
  ...
  override type Out =
    Either[IRBuilderError, (Option[CypherStatement], IRBuilderContext)]

  override def process(input: ast.Statement)(implicit
  ...
  private def getTuple(output: Out) =
    output match {
      case Left(error) =>
        throw IllegalStateException(s"Error during IR construction: $error")
      case Right((None, _)) =>
        throw IllegalStateException(s"Failed to construct IR")
      case Right((Some(q), ctx)) => q -> ctx
    }

  override def extract(output: Out): CypherStatement = getTuple(output)._1

  private def buildIR[R: _mayFail: _hasContext](
    s: ast.Statement
  ): Eff[R, Option[CypherStatement]] =
    s match {
        ...
        for {
          ...
          result <- {
            ...
            val schema = maybeSingleQuery.get.model.result match {
              case GraphResultBlock(_, irGraph) =>
                context.schemaFor(irGraph.qualifiedGraphName)
              case _ =>
                throw IllegalArgumentException(
                  "The query in CATALOG CREATE GRAPH must return a graph"
                )
            }
            val irQgn = QualifiedGraphName(qgn.parts)
            val statement = Some(
              CreateGraphStatement(
                IRCatalogGraph(irQgn, schema),
                maybeSingleQuery.get
              )
            )
  ...
  private def convertQueryPart[R: _mayFail: _hasContext](
    part: ast.QueryPart
    ...
    part match {
      case ast.SingleQuery(clauses) =>
        val plannedBlocks = for {
          context <- get[R, IRBuilderContext]
          blocks <- put[R, IRBuilderContext](
            context.resetRegistry
          ) >> clauses.toList.traverse(convertClause[R])
        } yield blocks
  ...
  private def convertClause[R: _mayFail: _hasContext](
    c: ast.Clause
  ): Eff[R, List[Block]] = {
    ...
    c match {
        ...
        for {
          ...
          blocks <- {
            ...
            val updatedContext = context
              .withWorkingGraph(irGraph)
              .registerGraph(generatedQgn, graph)
            put[R, IRBuilderContext](updatedContext) >> pure[R, List[Block]](
              List.empty
            )
          }
        } yield blocks
        ...
        for {
          ...
          result <- {
            ...
            val fromGraph = maybeParameterValue match {
              case Some(CypherString(paramValue)) =>
                ast.GraphLookup(
                  ast.CatalogName(QualifiedGraphName.splitQgn(paramValue))
                )(p.position)
              case Some(other) =>
              ...
              case None =>
                throw ParsingException(
                  s"No parameter ${p.name} was specified ${p.position}"
                )
            }
      ...
      case ast.Match(optional, pattern, _, astWhere) =>
        for {
          ...
          blocks <- {
            ...
            val block =
              MatchBlock(after, pattern, given, optional, context.workingGraph)

            val typedOutputs = typedMatchBlock.outputs(block)
            val updatedRegistry = blockRegistry.register(block)
            val updatedContext = context
              .withBlocks(updatedRegistry)
              .withFields(typedOutputs)
              .registerConnections(pattern.topology)
            put[R, IRBuilderContext](updatedContext) >> pure[R, List[Block]](
              List(block)
            )
          }
        ...
        for {
          ...
          refs <- {
            val (projectRef, projectReg) =
              registerProjectBlock(
                context,
                fieldExprs,
                given,
                context.workingGraph,
                distinct = distinct
              )
            val appendList = (list: List[Block]) => pure[R, List[Block]](projectRef +: list)
            val orderAndSliceBlock =
              registerOrderAndSliceBlock(orderBy, skip, limit)
            val updatedContext =
              context.updateKnownConnections(fieldExprs).withBlocks(projectReg)
            put[R, IRBuilderContext](
              updatedContext
            ) >> orderAndSliceBlock flatMap appendList
          }
        } yield refs
      ...
      case ast.With(_, ast.ReturnItems(_, items), _, _, _, None) =>
        for {
          ...
          blocks <- {
            ...
            val (projectBlock, updatedRegistry1) = registerProjectBlock(
              context,
              group,
              source = context.workingGraph,
              distinct = false
            )
            val after = updatedRegistry1.lastAdded.toList
            val aggregationBlock = AggregationBlock(
              after,
              Aggregations(agg.toSet),
              group.map(_._1).toSet,
              context.workingGraph
            )
            val updatedRegistry2 = updatedRegistry1.register(aggregationBlock)
            val updatedContext = context
              .updateKnownConnections(fieldExprs)
              .withBlocks(updatedRegistry2)
            put[R, IRBuilderContext](updatedContext) >> pure[R, List[Block]](
              List(projectBlock, aggregationBlock)
            )
          }
        } yield blocks
      ...
      case ast.Unwind(listExpression, variable) =>
        for {
          ...
          block <- {
            ...
            val block = UnwindBlock(
              context.blockRegistry.lastAdded.toList,
              binds,
              context.workingGraph
            )
      ...
      case ast.ConstructGraph(clones, creates, on, sets) =>
        for {
          ...
          explicitCloneItems <- clones
            .flatMap(_.items)
            .traverse(convertClone[R](_, qgn))

          createPatterns <- creates
            .map { case ast.CreateInConstruct(p: exp.Pattern) =>
              p
            }
            .traverse(convertPattern[R](_, Some(qgn)))

          setItems <- sets
            .flatMap { case ast.SetClause(s) =>
              s
            }
            .traverse(convertSetItem[R])

          refs <- {
            val onGraphs: List[QualifiedGraphName] =
              on.map(graph => QualifiedGraphName(graph.parts))
            val schemaForOnGraphUnion =
              onGraphs.foldLeft(PropertyGraphSchema.empty) { case (agg, next) =>
            ...
            val implicitCloneItems = createPattern.fields.filterNot { f =>
              f.cypherType.graph.get == qgn || explicitCloneItemMap.keys.exists(
                _.name == f.name
              )
            }
            ...
            val cloneSchema = schemaForElementTypes(
              context,
              cloneItemMap.values.map(_.cypherType).toSet
            )
            ...
            cloneItemMap.foreach {
              case (IRField(constructRelName), _: RelationshipVar) =>
                createPatterns.flatMap(_.topology).foreach {
                  case (relField @ IRField(patternFieldName), con: Connection)
                      if patternFieldName == constructRelName =>
                    val expectedConnection =
                      cloneContext.knownConnections.getOrElse(
                        relField,
                        throw IllegalArgumentException(
                          "correct source and target nodes",
                          con,
              ...
              cloneFieldAlias.cypherType match {
                case _: CTRelationship if !createPattern.fields.contains(cloneFieldAlias) =>
                  throw UnsupportedOperationException(
                    s"Can only clone relationship ${cloneFieldAlias.name} if it is also part of a CREATE pattern"
                  )
                case _ => ()
              }
            }

            val fieldsInNewPattern = createPattern.fields
              .filterNot(cloneItemMap.contains)

            val patternSchema = fieldsInNewPattern.foldLeft(cloneSchema) { case (acc, next) =>
              val newFieldSchema =
                schemaForNewField(next, createPattern, context)
              acc ++ newFieldSchema
            }

            val (patternSchemaWithSetItems, _) =
              setItems.foldLeft(patternSchema -> Map.empty[Var, CypherType]) {
                case ((currentSchema, rewrittenVarTypes), setItem: SetItem) =>
                  setItem match {
                    case SetLabelItem(variable, labels) =>
                      val (existingLabels, existingQgn) = rewrittenVarTypes
                        .getOrElse(variable, variable.cypherType) match {
                        case CTNode(ls, qualifiedGraphName) =>
                          ls -> qualifiedGraphName
                        case other =>
                          throw UnsupportedOperationException(
                            s"SET label on something that is not a node: $other"
                          )
                      }
                      val labelsAfterSet = existingLabels ++ labels
                      val updatedSchema =
                        currentSchema.addLabelsToCombo(labels, existingLabels)
                      updatedSchema -> rewrittenVarTypes.updated(
                        variable,
                        CTNode(labelsAfterSet, existingQgn)
                      )
                    case SetPropertyItem(propertyKey, variable, setValue) =>
                      val propertyType = setValue.cypherType
                      val updatedSchema = currentSchema.addPropertyToElement(
                        propertyKey,
                        propertyType,
                        variable.cypherType
                      )
            ...
            val patternGraphSchema =
              schemaForOnGraphUnion ++ patternSchemaWithSetItems
            ...
            val updatedContext = context.resetKnownConnections
      ...
      case ast.ReturnGraph(None) =>
        for {
          ...
          refs <- {
            ...
            put[R, IRBuilderContext](
              context.resetKnownConnections.withBlocks(updatedRegistry)
            ) >> pure[R, List[Block]](List(returns))
          }
        } yield refs
        ...
        for {
          ...
          blocks1 <- {
            ...
            val orderAndSliceBlock =
              registerOrderAndSliceBlock(orderBy, skip, limit)
            put[R, IRBuilderContext](
              updatedContext.withBlocks(projectReg)
            ) >> orderAndSliceBlock flatMap appendList
          }
          ...
          blocks2 <- {
            ...
            val resultBlock = TableResultBlock(
              List(blocks1.last),
              orderedFields,
              context.workingGraph
            )
      ...
      case x =>
        error(IRBuilderError(s"Clause not yet supported: $x"))(
          List.empty[Block]
        )
    }
  }

  def schemaForElementTypes(
    context: IRBuilderContext,
  ...
  def schemaForElementType(
    context: IRBuilderContext,
    cypherType: CypherType
  ): PropertyGraphSchema = {
    val graphSchema = cypherType.graph
      .map(context.schemaFor)
      .getOrElse(context.workingGraph.schema)
  ...
  private def registerOrderAndSliceBlock[R: _mayFail: _hasContext](
    ...
    for {
      ...
      blocks <- {
        if (sortItems.isEmpty && skipExpr.isEmpty && limitExpr.isEmpty)
          pure[R, List[Block]](List())
        else {
          val blockRegistry = context.blockRegistry
          val after = blockRegistry.lastAdded.toList

          val orderAndSliceBlock = OrderAndSliceBlock(
            after,
            sortItems,
            skipExpr,
            limitExpr,
          ...
          put[R, IRBuilderContext](
            context.copy(blockRegistry = updatedRegistry)
          ) >> pure[R, List[Block]](List(orderAndSliceBlock))
        }
      }
    } yield blocks
  }
  ...
  private def convertClone[R: _mayFail: _hasContext](
    ...
    def convert(cypherType: CypherType, name: String): IRField =
      IRField(name)(cypherType.withGraph(qgn))
    ...
    item match {
      ...
      case ast.AliasedReturnItem(e, v) =>
        for {
          ...
          field <- {
            ...
            put[R, IRBuilderContext](context.withFields(Set(field))) >> pure[
              R,
              IRField
            ](field)
          }
      ...
      case _ =>
        throw IllegalArgumentException(
          s"${ast.AliasedReturnItem.getClass} or ${ast.UnaliasedReturnItem.getClass}",
          item.getClass
        )
    }
  }

  private def convertReturnItem[R: _mayFail: _hasContext](
    item: ast.ReturnItem
  ): Eff[R, (IRField, Expr)] = item match {
  ...
  private def convertPattern[R: _hasContext](
    ...
    for {
      ...
      result <- {
        ...
        val patternTypes = pattern.fields.foldLeft(context.knownTypes) { case (acc, f) =>
          acc.updated(exp.Variable(f.name)(InputPosition.NONE), f.cypherType)
        }
        put[R, IRBuilderContext](
          context.copy(knownTypes = patternTypes)
        ) >> pure[R, Pattern](pattern)
      }
    } yield result
  }

  private def convertExpr[R: _mayFail: _hasContext](
    e: Option[exp.Expression]
  ): Eff[R, Option[Expr]] =
    for {
      context <- get[R, IRBuilderContext]
    } yield e match {
  ...
  private def convertWhere[R: _hasContext](
    where: Option[ast.Where]
  ): Eff[R, Set[Expr]] = where match {
  ...
  private def convertRegistry[R: _mayFail: _hasContext]: Eff[R, Option[CypherQuery]] =
    for {
      ...
      val model = QueryModel(
        blocks.lastAdded.get.asInstanceOf[ResultBlock],
        context.parameters
      )
  ...
  private def convertSortItem[R: _mayFail: _hasContext](
    item: ast.SortItem
  ): Eff[R, SortItem] = {
    item match {
  ...
  private def schemaForNewField(
    field: IRField,
    pattern: Pattern,
    context: IRBuilderContext
  ): PropertyGraphSchema = {
    val baseFieldSchema = pattern.baseFields
      .get(field)
      .map { baseNode =>
    ...
    val newPropertyKeys: Map[String, CypherType] = pattern.properties
      .get(field)
    ...
    field.cypherType match {
      case CTNode(newLabels, _) =>
        val oldLabelCombosToNewLabelCombos =
          if (baseFieldSchema.labels.nonEmpty)
        ...
        val updatedPropertyKeys = oldLabelCombosToNewLabelCombos.map {
          case (oldLabelCombo, newLabelCombo) =>
            newLabelCombo -> (baseFieldSchema.nodePropertyKeys(
              oldLabelCombo
            ) ++ newPropertyKeys)
        }

        updatedPropertyKeys.foldLeft(PropertyGraphSchema.empty) {
          case (acc, (labelCombo, propertyKeys)) =>
            acc.withNodePropertyKeys(labelCombo, propertyKeys)
        }
      ...
      case CTRelationship(newTypes, _) if newTypes.size == 1 =>
        val possiblePropertyKeys = baseFieldSchema.relTypePropertyMap.values
          .map(_.keySet)
        ...
        PropertyGraphSchema.empty.withRelationshipPropertyKeys(
          newTypes.head,
          updatedPropertyKeys
        )
      ...
      case CTRelationship(newTypes, _) =>
        val actualTypes =
          if (newTypes.nonEmpty) newTypes else baseFieldSchema.relationshipTypes

        actualTypes.foldLeft(PropertyGraphSchema.empty) { case (acc, relType) =>
          acc.withRelationshipPropertyKeys(
            relType,
            baseFieldSchema.relationshipPropertyKeys(relType) ++ newPropertyKeys
          )
        }

      case other =>
        throw IllegalArgumentException("CTNode or CTRelationship", other)
    }
  }

  private def convertSetItem[R: _hasContext](
    p: ast.SetItem
  ): Eff[R, SetItem] = {
    p match {
      case ast.SetPropertyItem(
            exp.LogicalProperty(
              map: exp.Variable,
              exp.PropertyKeyName(propertyName)
            ),
        ...
        for {
          ...
          result <- {
            val setItem = SetPropertyItem(
              propertyName,
              variable.asInstanceOf[Var],
              convertedSetExpr
            )
      ...
      case ast.SetLabelItem(expr, labels) =>
        for {
          ...
          result <- {
            val setLabel: SetItem =
              SetLabelItem(variable.asInstanceOf[Var], labels.map(_.name).toSet)
