...
/**
  ...
  */
object GenericDataFrameInputExample extends App {
  ...
  val nodesDF = SocialNetworkDataFramesWithStringIds.nodes(spark)
  val relsDF = SocialNetworkDataFramesWithStringIds.rels(spark)
  ...
  val result = graph.cypher("MATCH (n:Person)-->(m) RETURN n.name, m.name")
...
object SocialNetworkDataFramesWithStringIds {
