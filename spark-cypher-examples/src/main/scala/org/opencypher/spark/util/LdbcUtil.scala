...
object LdbcUtil {
  ...
  def toGraphDDL(datasource: String, database: String)(implicit spark: SparkSession): String = {
    ...
    val nodeMappings = nodeTables
      .map(nodeTable => s"(${nodeTable.toNodeLabel}) FROM $nodeTable")

    val edgeMappings = edgeTables
      .map(edgeTable => edgeTable -> edgeTable.split("_").toList)
      .map {
        case (edgeTable, startTable :: relType :: endTable :: Nil) =>
          ...
          relType -> s"FROM $edgeTable edge START NODES $startNodes END NODES $endNodes"

        case (table, _) =>
          throw new IllegalArgumentException(s"Unexpected table name: $table")
      }
      .groupBy(_._1)
