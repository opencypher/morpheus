...
object operations {
  ...
  implicit final class RichCAPSGraph(val graph: CAPSGraph) extends AnyVal {
    ...
    def filter(subject: CAPSRecords, expr: Expr, parameters: Map[String, CAPSValue] = Map.empty)(
        implicit caps: CAPSSessionImpl): CAPSRecords =
      caps.filter(graph, subject, expr, parameters)

    def select(subject: CAPSRecords, fields: IndexedSeq[Var], parameters: Map[String, CAPSValue] = Map.empty)(
        implicit caps: CAPSSessionImpl): CAPSRecords =
      caps.select(graph, subject, fields, parameters)

    def project(subject: CAPSRecords, expr: Expr, parameters: Map[String, CAPSValue] = Map.empty)(
        implicit caps: CAPSSessionImpl): CAPSRecords =
      caps.project(graph, subject, expr, parameters)

    def alias(subject: CAPSRecords, alias: (Expr, Var), parameters: Map[String, CAPSValue] = Map.empty)(
        implicit caps: CAPSSessionImpl): CAPSRecords =
