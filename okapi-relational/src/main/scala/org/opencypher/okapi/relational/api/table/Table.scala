...
/**
  * Main abstraction of a relational backend. A table represents a relation in terms of relational
  * algebra and exposes relational and additional auxiliary operators. Operators need to be
  * implemented by the specific backend (e.g. morpheus-spark-cypher).
  *
  * @tparam T
...
trait Table[T <: Table[T]] extends CypherTable {
  ...
  /**
    * If supported by the backend, calling that operator caches the underlying table within the
    * backend runtime.
    *
    * @return
  ...
  /**
    * Returns a table containing only the given columns. The column order within the table is
  ...
  /**
    * Joins the current table with the given table on the specified join columns using equi-join
    * semantics.
    *
    * @param other
    *   table to join
  ...
  /**
    * Computes the union of the current table and the given table. Requires both tables to have
  ...
  /**
    * Groups the rows within the table by the given query variables. Additionally a set of
    * aggregations can be performed on the grouped table.
    *
    * @param by
    ...
    */
  def group(by: Set[Var], aggregations: Map[String, Aggregator])(implicit
    header: RecordHeader,
    parameters: CypherMap
  ): T

  /**
    * Returns a table with additional expressions, which are evaluated and stored in the specified
    * columns.
    *
    * @note
