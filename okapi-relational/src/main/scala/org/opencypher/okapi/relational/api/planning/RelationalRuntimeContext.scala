...
case class RelationalRuntimeContext[T <: Table[T]](
  ...
  var queryLocalCatalog: Map[QualifiedGraphName, RelationalCypherGraph[T]] =
    Map.empty[QualifiedGraphName, RelationalCypherGraph[T]]
  ...
  def resolveGraph(qgn: QualifiedGraphName): RelationalCypherGraph[T] =
    queryLocalCatalog.get(qgn) match {
      case None =>
        sessionCatalog(qgn).getOrElse(
          throw IllegalArgumentException(s"a graph at $qgn")
        )
      case Some(g) => g
    }
}
