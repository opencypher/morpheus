...
//        }
//    }
//    session.records.from(targetHeader, dataWithMissingColumns)
//  }
//
//  def distinct(fields: Var*): Records = {
//    session.records.from(header, table.distinct(fields.flatMap(header.expressionsFor).map(header.column).sorted: _*))
//  }
}
