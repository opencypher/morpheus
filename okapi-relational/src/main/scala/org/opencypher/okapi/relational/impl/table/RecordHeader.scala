...
object RecordHeader {
  ...
  def from[T <: Expr](expr: T, exprs: T*): RecordHeader =
    empty.withExprs(expr, exprs: _*)

  def from[T <: Expr](exprs: Set[T]): RecordHeader = empty.withExprs(exprs)

  def from[T <: Expr](exprs: Seq[T]): RecordHeader =
    from(exprs.head, exprs.tail: _*)
  ...
  def from(v: Var): RecordHeader = v.cypherType match {
    ...
    case other =>
      throw IllegalArgumentException("A node or relationship variable", other)
  }
}
...
case class RecordHeader(exprToColumn: Map[Expr, String]) {
  ...
  def returnItems: Set[ReturnItem] =
    expressions.flatMap(_.owner).collect { case r: ReturnItem => r }
  ...
  def column(expr: Expr): String = expr match {
    ...
    case _ =>
      exprToColumn.getOrElse(
        expr,
        throw IllegalArgumentException(
          s"Header does not contain a column for $expr.\n\t${this.toString}"
        )
      )
  }
  ...
  def expressionsFor(expr: Expr): Set[Expr] = {
    expr match {
      case v: Var =>
        if (exprToColumn.contains(v)) ownedBy(v) + v else ownedBy(v)
  ...
  def aliasesFor(expr: Expr): Set[Var] = {
    val aliasesFromHeader: Set[Var] = getColumn(expr) match {
      case None => Set.empty
      case Some(col) =>
        exprToColumn.collect { case (k: Var, v) if v == col => k }.toSet
    }
  ...
  def idExpressions(v: Var): Set[Expr] =
    idExpressions().filter(_.owner.get == v)
  ...
  def labelsFor(n: Var): Set[HasLabel] = {
    ownedBy(n).collect { case l: HasLabel =>
      l
    }
  }

  def typesFor(r: Var): Set[HasType] = {
    ownedBy(r).collect { case t: HasType =>
      t
    }
  }

  def startNodeFor(r: Var): StartNode = {
    ownedBy(r).collectFirst { case s: StartNode =>
      s
    }.get
  }

  def endNodeFor(r: Var): EndNode = {
    ownedBy(r).collectFirst { case e: EndNode =>
      e
    }.get
  }

  def propertiesFor(v: Var): Set[Property] = {
    ownedBy(v).collect { case p: Property =>
      p
    }
  }
  ...
  def nodeVars[T >: NodeVar <: Var]: Set[T] = {
    exprToColumn.keySet.collect { case v: NodeVar =>
      v
    }
  }
  ...
  def relationshipVars[T >: RelationshipVar <: Var]: Set[T] = {
    exprToColumn.keySet.collect { case v: RelationshipVar =>
      v
    }
  }
  ...
  def nodesForType[T >: NodeVar <: Var](
    nodeType: CTNode,
  ...
  def relationshipsForType[T >: RelationshipVar <: Var](
    ...
    relationshipVars[T].filter { relVar =>
      val physicalTypes = typesFor(relVar).map { case HasType(_, RelType(name)) =>
        name
      }
      ...
      possibleTypes.isEmpty || (physicalTypes ++ logicalTypes).exists(
        possibleTypes.contains
      )
    }
  }
  ...
  def select[T <: Expr](exprs: Set[T]): RecordHeader = {
    ...
    val selectMappings =
      headerWithAliases.exprToColumn.filterKeys(selectExpressions.contains)
    RecordHeader(selectMappings)
  }

  def withColumnsReplaced[T <: Expr](
    replacements: Map[T, String]
  ): RecordHeader = {
    RecordHeader(exprToColumn ++ replacements)
  }

  def withColumnsRenamed[T <: Expr](renames: Seq[(T, String)]): RecordHeader = {
    renames.foldLeft(this) { case (currentHeader, (expr, newColumn)) =>
      currentHeader.withColumnRenamed(expr, newColumn)
    }
  }
  ...
  private[opencypher] def newConflictFreeColumnName(
    expr: Expr,
    usedColumnNames: Set[String] = columns
  ): String = {
    @tailrec def recConflictFreeColumnName(candidateName: String): String = {
      if (usedColumnNames.contains(candidateName))
        recConflictFreeColumnName(s"_$candidateName")
      else candidateName
    }
        ...
        exprToColumn.get(expr) match {
          ...
          case None =>
            ...
            val exprsToAdd: Set[Expr] = expr.owner match {
              ...
              case Some(exprOwner) =>
                aliasesFor(exprOwner).map(alias => expr.withOwner(alias))
            }

            exprsToAdd.foldLeft(this) { case (current, e) =>
              current.addExprToColumn(e, newColumnName)
            }
        }
    }
  }
  ...
  def withExprs[T <: Expr](expr: T, exprs: T*): RecordHeader =
    (expr +: exprs).foldLeft(this)(_ withExpr _)
  ...
  def withAlias(expr: AliasExpr): RecordHeader = {
    ...
    to match {
      ...
      case elementExpr: Var if exprToColumn.contains(to) =>
        ...
        ownedBy(elementExpr)
          .filterNot(_ == elementExpr)
          .foldLeft(withElementExpr) { case (current, nextExpr) =>
            current.addExprToColumn(
              nextExpr.withOwner(alias),
              exprToColumn(nextExpr)
            )
          }

      // Non-element case
      case e if exprToColumn.contains(e) =>
        addExprToColumn(alias, exprToColumn(e))

      // No expression to alias
      case other =>
        throw IllegalArgumentException(
          s"An expression in $this",
          s"Unknown expression $other"
        )
    }
  }
  ...
  def join(other: RecordHeader): RecordHeader = {
    ...
    if (expressionOverlap.nonEmpty) {
      throw IllegalArgumentException(
        "two headers with non overlapping expressions",
        s"overlapping expressions: $expressionOverlap"
      )
    }
    ...
    if (columnOverlap.nonEmpty) {
      throw IllegalArgumentException(
        "two headers with non overlapping columns",
        s"overlapping columns: $columnOverlap"
      )
    }
  ...
  def union(other: RecordHeader): RecordHeader = {
    ...
    if (varOverlap != vars) {
      throw IllegalArgumentException(
        "two headers with the same variables",
        s"$vars and ${other.vars}"
      )
    }
  ...
  def ++(other: RecordHeader): RecordHeader = {
    val result = (exprToColumn ++ other.exprToColumn).map { case (key, value) =>
      val leftCT =
        exprToColumn.keySet.find(_ == key).map(_.cypherType).getOrElse(CTVoid)
      val rightCT = other.exprToColumn.keySet
        .find(_ == key)
        .map(_.cypherType)
        .getOrElse(CTVoid)
      ...
      val resultExpr = (key, leftCT, rightCT) match {
        ...
        case (v: Var, l: CTRelationship, r: CTRelationship) =>
          Var(v.name)(l.join(r))
        case (_, l, r) if l.subTypeOf(r) =>
          other.exprToColumn.keySet.collectFirst { case k if k == key => k }.get
  ...
  def --[T <: Expr](expressions: Set[T]): RecordHeader = {
    ...
    val updatedExprToColumn = exprToColumn.filterNot { case (e, _) =>
      expressionToRemove.contains(e)
    }
