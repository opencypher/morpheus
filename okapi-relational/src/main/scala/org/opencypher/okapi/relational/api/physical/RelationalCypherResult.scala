...
case class RelationalCypherResult[T <: FlatRelationalTable[T]](
  ...
  override def getRecords: Option[CypherRecords] = maybeRelational.map(op => session.records.from(op.header, op.table))
  ...
  override def plans: CypherQueryPlans = QueryPlans(maybeLogical, maybeRelational)
}
